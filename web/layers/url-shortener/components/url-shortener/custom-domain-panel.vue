...
<template>
	<div
		class="flex flex-col w-full max-w-xl gap-4 border border-[hsl(240,11%,18%)] bg-[hsl(240,11%,9%)] rounded-2xl p-4 shadow-[0px_0px_30px_hsl(240,11%,6%)]"
	>
		...
		<div v-if="!isAuthenticated" class="rounded-xl border border-[hsl(240,11%,18%)] bg-[hsl(240,11%,12%)] p-4">
			<p class="text-sm text-[hsl(240,11%,70%)]">
				Sign in to connect and manage your custom domain.
			</p>
			<UiButton class="mt-3" @click="userStore.login">Login</UiButton>
		</div>

		<div v-else class="space-y-4">
			<div v-if="isCustomDomainLoading" class="rounded-xl border border-[hsl(240,11%,18%)] bg-[hsl(240,11%,12%)] p-4">
				<p class="text-sm text-[hsl(240,11%,70%)]">Loading domain status...</p>
			</div>

			<div v-else-if="hasCustomDomain" class="rounded-xl border border-[hsl(240,11%,18%)] bg-[hsl(240,11%,12%)] p-4">
				...
				<div class="mt-4 space-y-2 text-sm text-[hsl(240,11%,70%)]">
					<div class="flex flex-col gap-1">
						<span class="text-xs uppercase tracking-wide text-[hsl(240,11%,55%)]">CNAME target</span>
					...
					<p>
						...
						<span class="font-mono text-[hsl(240,11%,85%)]">https://{{ previewDomain }}/your-slug</span>
					</p>
				</div>
