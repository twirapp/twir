...
<template>
	<UiPopover v-model:open="open">
		...
		<UiPopoverContent class="w-[400px] p-0">
			<UiCommand :filter-function="(list: any) => list">
				...
				<UiCommandList>
					<UiCommandEmpty v-if="!isSearching && displayedCategories.length === 0">
						{{ searchQuery ? 'No category found.' : 'Start typing to search...' }}
					</UiCommandEmpty>
					<UiCommandEmpty v-else-if="isSearching"> Searching... </UiCommandEmpty>
					<UiCommandGroup v-if="displayedCategories.length > 0">
						<UiCommandItem
