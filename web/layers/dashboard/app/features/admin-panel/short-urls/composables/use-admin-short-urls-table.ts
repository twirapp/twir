...
import UsersTableCellUser from '~~/layers/dashboard/app/features/admin-panel/manage-users/ui/users-table-cell-user.vue'
import ShortUrlsActions from '~~/layers/dashboard/app/features/admin-panel/short-urls/ui/short-urls-actions.vue'
import {
	TABLE_ACCESSOR_KEYS,
} from '~~/layers/dashboard/app/features/community-users/composables/use-community-users-table.js'
...
export const useAdminShortUrlsTable = createGlobalState(() => {
	...
	const tableColumns = computed<ColumnDef<AdminShortUrl>[]>(() => [
		...
		},
		{
			...
			header: () => h('div', {}, 'Link'),
			cell: ({ row }) => {
				return h('a', { href: row.original.link, target: '_blank', class: 'underline' }, row.original.link)
			},
		},
		{
			accessorKey: 'views',
