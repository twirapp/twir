...
import {
	useMutationYouTubeBotSetupComplete,
...
const completeSetupMutation = useMutationYouTubeBotSetupComplete()
...
onMounted(async () => {
	...
	if (!callback.success || callback.data.error || !callback.data.code || !callback.data.state) {
		showFailure('YouTube authorization was not completed')
		return
	}
	...
	if (result.error || !result.data?.youtubeBotSetupComplete) {
		showFailure(result.error?.message ?? 'Unable to complete YouTube bot setup')
		return
	}

	new BroadcastChannel(youtubeBotSetupBroadcastChannelName).postMessage('refresh')
	toast.success('YouTube bot connected')
	window.close()
})
</script>
