...
const postCode = integrations.streamlabsPostCode()
...
onMounted(async () => {
	...
	try {
		const result = await postCode.executeMutation({ input: { code, state } })
		if (result.error || !result.data?.streamlabsPostCode) {
			fail('imports.errors.callbackRequest')
			return
		}
		integrations.broadcastRefresh()
		closeWindow()
	} catch {
		fail('imports.errors.callbackRequest')
	}
...
<template>
	<div class="flex h-full items-center justify-center p-6">
		<Alert v-if="errorMessage" variant="destructive" class="max-w-md">
			...
			<AlertTitle>{{ t('imports.errors.callbackTitle', { provider: 'Streamlabs' }) }}</AlertTitle>
		...
		<div v-else class="flex items-center gap-3 text-muted-foreground">
			...
			<span>{{ t('imports.callback.connecting', { provider: 'Streamlabs' }) }}</span>
		</div>
	</div>
</template>
