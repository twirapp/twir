			...
			<CardContent class="space-y-6">
				...
				>
					<div class="border-t pt-4">
						<h3 class="mb-3 text-lg font-semibold">{{ t('overlays.obs.connectionInfo') }}</h3>
						<div class="grid grid-cols-1 gap-4 md:grid-cols-3">
							<Card class="bg-muted/50">
								<CardContent class="pt-4">
									<div class="text-2xl font-bold">{{ settings.scenes?.length ?? 0 }}</div>
									<p class="text-muted-foreground text-sm">{{ t('overlays.obs.scenesCount') }}</p>
								</CardContent>
							</Card>
							<Card class="bg-muted/50">
								<CardContent class="pt-4">
									<div class="text-2xl font-bold">{{ settings.sources?.length ?? 0 }}</div>
									<p class="text-muted-foreground text-sm">{{ t('overlays.obs.sourcesCount') }}</p>
								</CardContent>
							</Card>
							<Card class="bg-muted/50">
								<CardContent class="pt-4">
