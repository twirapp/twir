	...
	<PageLayout show-back back-redirect-to="/dashboard/integrations">
		...
		<template #content>
			<div v-if="guilds.length === 0" class="flex flex-col items-center justify-center py-12 gap-4">
				...
				<UiButton variant="default" @click="handleConnectGuild">
					<Plus class="h-4 w-4 mr-2" />
					{{ t('integrations.discord.connectGuild') }}
				</UiButton>
			</div>
			...
			<div v-else class="flex flex-col gap-6">
				<UiTabs v-model="activeGuildId" class="w-full">
					<UiTabsList class="mb-4">
						<UiTabsTrigger v-for="guild in guilds" :key="guild.id" :value="guild.id">
					...
					<UiTabsContent v-for="guild in guilds" :key="guild.id" :value="guild.id">
							...
							<div class="border border-destructive/50 rounded-lg p-4">
								...
								<UiAlertDialog>
									...
									<UiAlertDialogContent>
										<UiAlertDialogHeader>
											<UiAlertDialogTitle>{{ t('deleteConfirmation.text') }}</UiAlertDialogTitle>
										...
										<UiAlertDialogFooter>
											...
											<UiAlertDialogAction @click="handleDisconnectGuild(guild.id)">
