...
<template>
	<div class="flex flex-col gap-3">
		...
		<UiSeparator />
		<UiAlert v-if="!isSomeSongIntegrationEnabled" variant="destructive">
			<AlertTriangleIcon class="h-4 w-4" />
			<UiAlertTitle>No enabled song integrations!</UiAlertTitle>
			<UiAlertDescription>
		...
		>
			...
			<UiAlert v-if="!entities?.nowPlayingOverlays.length" class="mt-2">
				<UiAlertTitle>No overlays!</UiAlertTitle>
				<UiAlertDescription> Create new overlay for edit settings </UiAlertDescription>
			</UiAlert>
			<TabsContent
				v-for="(overlay, index) of entities?.nowPlayingOverlays"
				:key="overlay.id"
