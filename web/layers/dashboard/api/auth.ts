...
export const useUnlinkPlatformAccount = () => {
	...
	return {
		executeMutation: (variables: { platform: string }) =>
...
export const useDashboard = createGlobalState(() => {
	const { executeMutation: mutationSetDashboard } = useMutation(
		graphql(`
...
export function useUserAccessFlagChecker(flag: ChannelRolePermissionEnum) {
	...
	return computed(() => {
		...
		const dashboard = profile.value?.availableDashboards.find((d: { id: string }) => {
			return d.id === profile.value?.selectedDashboardId
		})
...
export async function userAccessFlagChecker(flag: ChannelRolePermissionEnum) {
	...
	if (data?.authenticatedUser?.isBotAdmin) return true
	if (!data || !data?.authenticatedUser?.selectedDashboardId) return false

	const dashboard = data.authenticatedUser.availableDashboards.find((d: { id: string }) => {
		return d.id === data.authenticatedUser.selectedDashboardId
	})
	if (!dashboard) return false
