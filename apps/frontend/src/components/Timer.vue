...
type TimerType = SetOptional<
  Omit<Timer, 'responses' | 'channelId'> & { edit?: boolean; responses: string[] },
  'id'
>;
...
const responses = ref(props.timer.responses);
...
async function saveTimer() {
  ...
  if (timer.value.id) {
    const request = await api.put(
      `/v1/channels/${selectedDashboard.value.channelId}/timers/${timer.value.id}`,
      timerData,
    );
    data = request.data;
  } else {
    const request = await api.post(
      `/v1/channels/${selectedDashboard.value.channelId}/timers`,
      timerData,
    );
    data = request.data;
  }
...
<template>
  <div class="p-2">
    <Form v-slot="{ errors }" @submit="saveTimer">
      <div
        ...
        <div class="col-span-2 mt-1">
          <span class="flex items-center label">
            ...
            <span
              ...
              @click="responses.push({ text: '', isAnnounce: true })">
              <Add />
            </span>
          </span>
            ...
            <div
              ...
              <div class="flex flex-wrap items-stretch relative">
                ...
                <div
                  ...
                  @click="responses?.splice(responseIndex, 1)">
                  <span
              ...
              <div class="flex form-check justify-between mt-2">
                <label class="form-check-label inline-block" for="isAnnounce">{{
                  t('pages.timers.card.useAnnounce')
                }}</label>

                <div class="form-switch">
                  <input
                    id="isAnnounce"
                    v-model="response.isAnnounce"
                    class="align-top appearance-none bg-contain bg-gray-300 bg-no-repeat cursor-pointer float-left focus:outline-none form-check-input h-5 rounded-full shadow w-9"
                    type="checkbox"
                    :disabled="!timer.edit"
                    role="switch" />
      ...
      <div
        class="flex flex-col lg:flex-row lg:justify-between lg:space-x-1 lg:space-y-0 mt-5 space-y-1">
        <div>
          <MyBtn
            v-if="!timer.edit"
            class="w-full"
          ...
          <MyBtn v-else class="lg:w-auto w-full" color="purple" @click="cancelEdit">
            {{ t('buttons.cancel') }}
          </MyBtn>
        </div>
        <div v-if="timer.edit" class="flex md:flex-none space-x-1">
          <MyBtn v-if="timer.id" class="lg:w-auto w-1/2" color="red" @click="deleteTimer">
            {{ t('buttons.delete') }}
          </MyBtn>
          <MyBtn class="lg:w-auto w-1/2" color="green" type="submit">
            {{ t('buttons.save') }}
          </MyBtn>
        </div>
      </div>
