...
const filteredDashboards = computed(() => {
  return user.value?.dashboards
    .filter((c) =>
      searchFilter.value
        ? [c.twitchUser.login, c.twitchUser.display_name].some((s) =>
            s.includes(searchFilter.value),
          )
        : true,
    )
    .sort((a, b) =>
      a.twitchUser.id === user.value?.id
        ? -1
        : a.twitchUser.login.localeCompare(b.twitchUser.login),
    );
});
    ...
    <div>
      <div class="dropdown relative">
        ...
        <img
          ...
          data-bs-toggle="dropdown"
          aria-expanded="false"
          :src="selectedDashboard?.twitchUser?.profile_image_url ?? user?.profile_image_url"
          class="hover:cursor-pointer hover:opacity-60 rounded-full w-9"
          alt="Avatar" />

        <div
          class="absolute bg-[#202020] bg-clip-padding border-none dropdown-menu float-left hidden list-none m-0 min-w-max mt-1 px-2 py-2 rounded text-base text-left w-64 z-50"
          ...
          <div
            ...
            <input
              ...
              type="text"
              class="bg-clip-padding bg-white block border border-gray-300 border-solid ease-in-out focus:bg-white focus:border-blue-600 focus:outline-none focus:text-gray-700 font-normal form-control m-0 px-3 py-0.5 rounded text-base text-gray-700 transition w-full"
              placeholder="Search..." />
            <div
              ...
              <span
                ...
                <img
                  ...
                  :src="
                    dashboard?.twitchUser?.profile_image_url ??
                    dashboard.twitchUser?.profile_image_url
                  " />
                <span class="ml-4"
                  >{{ dashboard.twitchUser?.display_name }}
                  <span
          ...
          <div class="flex flex-col space-y-1 w-full">
            ...
            <MyBtn color="red" @click="logOut"> Logout </MyBtn>
          </div>
        </div>
      </div>
    </div>
