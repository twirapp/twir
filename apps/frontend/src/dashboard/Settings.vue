...
import noAvatar from '@/assets/twitch_no_avatar.jpg';
...
type Entity = {
  ...
  userId: string;
  channelId: string;
  twitchUser?: HelixUserData;
};
const user = useStore(userStore);
const newMember = ref('');
const dashboardEntities = ref<Array<Entity>>();
const { data } = useUpdatingData(`/v1/channels/{dashboardId}/settings/dashboard-access`);

watch(data, (v) => {
  dashboardEntities.value = v;
});

async function deleteEntity(id: string) {
  const response = await api.delete(
    `/v1/channels/${selectedDashboard.value.channelId}/settings/dashboard-access/${id}`,
  );
  // execute();

  if (response.status === 200) {
    dashboardEntities.value = dashboardEntities.value?.filter((v) => v.id != id);
  }
}
...
async function addMember() {
  ...
  if (response.status === 200) {
    dashboardEntities.value?.push(response.data);
    newMember.value = '';
  }
...
<template>
  <div class="m-1.5 md:m-3">
    <div class="grid grid-cols-1 md:grid-cols-2">
      <div class="block card mb-[0.5rem] pb-0.5 rounded shadow text-white">
        <h2
        ...
        <div>
          <div class="my-4 rounded text-base">
            <p v-if="!dashboardEntities?.length" class="text-center">
            ...
            <ul
              ...
              <li
                v-for="entity of dashboardEntities"
                :key="entity.id"
                class="bg-transparent block font-normal px-4 py-2 text-sm w-full whitespace-nowrap"
                :class="{
                  'hover:bg-[#121212]': entity.userId !== user?.id,
                  'bg-[#121212]': entity.userId === user?.id,
                }">
                <div
                  class="flex items-center justify-between"
                  :class="{ 'cursor-pointer': entity.id !== user?.id }"
                  @click="entity.userId !== user?.id ? deleteEntity(entity.id) : null">
                  <div>
                    <img
                      class="inline rounded-full w-6"
                      :src="entity.twitchUser?.profile_image_url ?? noAvatar" />
                    <span class="ml-4">{{ entity.twitchUser?.display_name || entity.userId }}</span>
                  </div>
                  <div v-if="entity.userId !== user?.id">
                    <Remove />
                  </div>
                  <div v-if="entity.userId === user?.id">
                    <span
          ...
          <div class="flex flex-wrap items-stretch relative w-full">
            ...
            <div class="cursor-pointer flex" @click="addMember">
              <span
                class="bg-green-600 flex hover:bg-green-700 items-center leading-normal px-3 rounded-r text-grey-dark text-sm whitespace-no-wrap"
                ><Add
              /></span>
            </div>
          </div>
