...
func (c *Client) ValidateToken(accessToken string) (bool, *ValidateTokenResponse, error) {
	...
	c.SetUserAccessToken(accessToken, nil)
	defer c.SetUserAccessToken(currentToken, nil)
...
func (c *Client) IsTokenExpired() bool {
	if c.TokenExpiresIn == nil || c.TokenObtainmentTimestamp == nil {
		return false
	}

	curr := time.Now().UnixMilli()

	expires := *c.TokenExpiresIn
	timestamp := *c.TokenObtainmentTimestamp
	isExpired := curr > (timestamp + int64(expires))

	return isExpired
}

func (c *Client) RefreshToken() error {
	if c.opts.UserRefreshToken == "" || c.opts.AppAccessToken == "" {
		return errors.New("cannot refresh token because not of any tokens are provided")
	}

	if c.opts.AppAccessToken == "" {
		newToken, err := c.RequestAppAccessToken(nil)

		if err != nil {
			return err
		}

		c.opts.AppAccessToken = newToken.Data.AccessToken
		c.TokenExpiresIn = &newToken.Data.ExpiresIn
		t := time.Now().Unix()
		c.TokenObtainmentTimestamp = &t
		if c.opts.OnRefresh != nil {
			(*c.opts).OnRefresh(newToken.Data.AccessToken, &newToken.Data.RefreshToken, newToken.Data.ExpiresIn)
		}
	}
	if c.opts.UserRefreshToken != "" {
		newToken, err := c.RefreshUserAccessToken(c.opts.UserRefreshToken)

		if err != nil {
			return err
		}
		c.opts.UserAccessToken = newToken.Data.AccessToken
		c.opts.UserRefreshToken = newToken.Data.RefreshToken
		c.TokenExpiresIn = &newToken.Data.ExpiresIn
		t := time.Now().Unix()
		c.TokenObtainmentTimestamp = &t
		if c.opts.OnRefresh != nil {
			c.opts.OnRefresh(newToken.Data.AccessToken, nil, newToken.Data.ExpiresIn)
		}
	}
