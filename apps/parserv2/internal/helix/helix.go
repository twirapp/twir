...
type Client struct {
	...
	TokenExpiresIn           *int
	TokenObtainmentTimestamp *int64
}
...
type Options struct {
	...
	OnRefresh func(newAccess string, newRefresh *string, expiresIn int)
}
...
func (c *Client) sendRequest(method, path string, respData, reqData interface{}, hasJSONBody bool) (*Response, error) {
	...
	if c.IsTokenExpired() && (c.opts.AppAccessToken != "" || c.opts.UserRefreshToken != "") {
		err := c.RefreshToken()
...
func (c *Client) SetAppAccessToken(accessToken string, expiresIn *int) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Client) SetUserAccessToken(accessToken string, expiresIn *int) {
	c.mu.Lock()
	defer c.mu.Unlock()
