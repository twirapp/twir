...
var Store twiri18n.LocalesStore = twiri18n.LocalesStore{"en": map[string]map[string]map[string]string{"commands": map[string]map[string]string{"nuke": map[string]string{"errors.cannot_get_handeled_messages": `Cannot get handled messages`, "errors.cannot_delete_messages": `Cannot delete messages`, "errors.timeout_duration": `Duration of timeout cannot be longer than 2 weeks`, "errors.parse_duration": `Cannot parse duration`, "errors.invalid_duration": `Invalid duration. Examples: !nuke 10m phrase, !nuke 10 phrase, !nuke 1h5m phrase`, "hints.nukeTimeArgName": `time, examples: 10m, 10, 1h5m`, "errors.cannot_get_users_stats": `Cannot get users stats`}, "songrequest": map[string]string{"errors.remove_song_from_queue": `Cannot remove song from queue`, "errors.update_song": `Cannot update song`, "info.no_requested_songs": `You haven't requested any song`, "info.delete": `Song {songTitle} deleted from queue`, "errors.get_users_count": `Cannot get online users count`, "errors.get_settings": `Cannot get song requests settings`, "errors.get_songs_from_queue": `Cannot get songs from queue`, "errors.get_current_queue_count": `Cannot get current queue count`, "info.only_count_songs": `There is only {songsCount} songs`, "errors.get_latest_song": `Cannot get latest song`, "errors.get_current_vote": `Cannot get current vote`, "errors.get_current_song": `Cannot get current song`, "validate.errors.restrictions_on_user": `There are restrictions on user, but i cannot find you in db, sorry :(`, "errors.get_votes_count": `Cannot get votes count`, "errors.search_song": `Cannot search song`, "info.song_skipped": `Song {songTitle} skipped`, "validate.errors.need_follow": `For request song you need to be a followed`, "validate.errors.internal_error": `Internal error when checking follow`, "errors.not_found": `Current song not found`}, "tts": map[string]string{"errors.sending_to_tts": `Error while sending message to tts service`, "errors.not_configured": `TTS is not configured for this channel`, "info.enabled": `TTS enabled`, "errors.while_getting_voices": `Error while getting voices`, "info.no_voices": `No voices available`, "errors.while_disable": `Error while disabling tts`, "errors.while_enable": `Error while enabling tts`, "info.change_pitch": `Pitch changed to {newPitch}`, "info.change_voice": `Voice changed to {newVoice}`, "info.current_volume": `Current volume: {ttsVolume}`, "info.voice_disallowed": `Voice {voiceName} is disallowed for usage`, "info.change_rate": `Rate changed to {newRate}`, "info.rate": `Global rate: {globalRate} | Your rate: {userRate}`, "info.voice": `Global voice: {globalVoice} | Your voice: {userVoice}`, "info.change_volume": `TTS volume changed to {userVolume}`, "info.disabled": `TTS disabled`, "info.pitch": `Global pitch: {globalPitch} | Your pitch: {userPitch}`}, "7tv": map[string]string{"errors.emoteset_not_found": `Emote set {emoteName} not found`, "remove.emote_remove": `✅ Emote {emoteName} removed`, "errors.emote_not_found": `Emote {emoteName} not found.`, "errors.emoteset_broadcaster_not_active": `❌ No active emote set for broadcaster`, "hints.copySetChannelName": `@channelName`, "hints.copySetNameOfSet": `Name of set to copy`, "errors.emote_failed_to_rename": `Failed to rename 7tv emote: {reason}`, "errors.emote_not_found_in_emoteset": `Emote {emoteName} not found in set {emoteSet}`, "add.emote_add": `✅ Emote added`, "hints.emoteForCopyArgName": `Name of emote to copy`, "errors.emote_failed_to_fetch": `Failed to fetch 7tv emote: {reason}`, "errors.emote_already_exist_in_channel": `❌ Emote {emoteName} already exists in this channel`, "errors.emote_not_found_in_channel": `❌ Emote {emoteSearch} not found in target channel`, "errors.profile_failed_to_get": `Failed to get 7tv profile: {reason}`, "rename.emote_rename": `✅ Emote {oldEmoteName} renamed to {newEmoteName}"`, "emote_info.response": `{name}: {link} · Added by @{addedByUserName} {addedByTime} ago · Author {emoteAuthor}`, "profile_info.response": `{profileName} · Paint: {paintName} ({unlockedPaints} unlocked) · Roles: {roles} · Editor for {editorCount} · Set: {emoteSetName} ({emoteSetCount}/{emoteSetCapacity}) · Created: {profileCreatedAt}`, "hints.emoteForAddArgAlias": `Optional alias`, "hints.emoteForCopyAlias": `Alias for emote`, "hints.emoteForAddArgLink": `Link or name`, "errors.emote_failed_to_remove": `Failed to remove 7tv emote: {reason}`, "errors.emoteset_not_active": `You don't have an active 7TV emote set`, "errors.profile_not_found": `7tv profile not found`, "errors.emote_failed_to_add": `Failed to add 7tv emote: {reason}`}, "shorturl": map[string]string{"success.short_url_created": `Short url: {url}`, "errors.cannot_create_short_url": `Cannot create short url. {error}`}, "shoutout": map[string]string{"response_offline": `Explore https://twitch.tv/{userName}’s world, last featured in 🎮 {categoryName} 📜 {title}!`, "errors.bot_have_no_permissions": `We have no permissions for shoutout. Streamer must re-authorize to bot dashboard.`, "response_online": `Join https://twitch.tv/{userName} shining in 🎮 {categoryName} 📜 {title} with 👁️ {viewers} viewers!`}, "stats": map[string]string{"info.watching_stream": `You watching stream for {userWatching}`, "info.watched": `{userWatched} watched`, "info.messages": `{userMessages} messages`, "info.emotes": `{userEmotes} used emotes`, "info.points": `{userPoints} used points`, "info.songs": `{userSongs} songs requestes`}, "categories_aliases": map[string]string{"errors.category_failed_to_get": `Failed to get caterogies`, "errors.alias_already_exists": `Alias {aliasName} already exists`, "errors.alias_not_found": `Category alias not found`, "errors.category_cannot_to_get": `Cannot get categories`, "errors.category_not_found": `Category not found`, "errors.category_required": `Alias and category are required`, "add.alias_add_to_category": `Category alias {aliasName} added with category {categoryName}`, "errors.alias_removed": `Category alias {aliasName} removed`, "errors.twitch_client_cannot_to_create": `Cannot create twitch client`, "errors.game_cannot_to_get": `Cannot get games`, "errors.category_failed_to_create": `Failed to create category`, "errors.category_cannot_delete": `Cannot delete category`, "errors.alias_empty": `No categories aliases created`}, "channel": map[string]string{"hints.gameArgName": `Category name or created category alias`, "errors.history_title_message": `Cannot get history of titles from database: {reason}`, "errors.broadcaster_twitch_api_client": `Cannot create broadcaster twitch api client: {reason}`, "errors.channel_not_found": `Channel not found`, "errors.category_not_found": `Category not found`, "errors.category_cannot_change": `Cannot change category`, "add.category_change": `✅ {categoryName}`, "errors.channel_cannot_get_information": `Cannot get channel information`, "errors.category_cannot_get_error": `Cannot get category: {errorMessage}`, "errors.history_game_message": `Cannot find used games in database`, "errors.game_not_found": `Game not found on twitch`, "errors.broadcaster_twitch_client_cannot_create": `Cannot create broadcaster twitch client`, "errors.alias_cannot_get_category": `Cannot get category aliases`, "errors.category_cannot_change_error": `Cannot change category: {errorMessage}`, "errors.category_cannot_get": `Cannot get category`}, "chat_wall": map[string]string{"errors.duration_cannot_parse": `Cannot parse duration`, "hints.deletePhraseArgName": `Phrase to delete`, "errors.chat_wall_not_found": `Chat wall {errorPhrase} not found or already stopped`, "errors.invalid_duration": `Invalid duration. Cannot be longer 2w Examples: 10m, 10, 1h5m`, "errors.long_duration_timeout": `Duration of timeout cannot be longer than 2 weeks`, "start.chat_wall_start": `✅ Chat wall started for 10 minutes, you can stop it with !chat wall stop {chatWallPhrase}`, "stop.chat_wal_stop": `✅ Chat wall {chatWallPhrase} stopped`, "hints.banPhraseArgName": `Phrase to ban`, "hints.timeoutDurationArgName": `Time. Examples: 10m, 10, 1h5m`, "hints.timeoutPhraseArgName": `Phrase to ban`}, "clip": map[string]string{"empty_clip_url": `Clip URL is empty, please try again`, "cannot_get_clip": `Cannot get created clip`, "clip_created": `Clip created: {url}`, "cannot_create_clip": `Cannot create clip`}, "games": map[string]string{"errors.duel_cannot_check_user": `Cannot check user in duel: {reason}`, "errors.voteban_cannot_set_vote": `Cannot set vote`, "errors.duel_cannot_set_global_cooldown": `Cannot set global cooldown: {reason}`, "errors.duel_with_bot": `You cannot duel with bot`, "errors.duel_cannot_save_data": `Cannot save duel data: {reason}`, "info.user_not_participate": `You are not participate in any duel`, "errors.voteban_cannot_find_settings": `Cannot find voteban settings`, "errors.voteban_cannot_find_user": `Cannot find target user`, "errors.duel_cannot_validate_participants": `Cannot validate participants`, "errors.voteban_cannot_lock": `Cannot lock voteban`, "errors.roulette_cannot_send_death_message": `Cannot send death message`, "errors.roulette_cannot_send_initial_message": `Cannot send initial message`, "errors.voteban_cannot_set_vote_expiration": `Cannot set vote expiration`, "errors.duel_with_streamer": `You cannot duel with streamer`, "errors.duel_cannot_check_cooldown": `Cannot check duel cooldown: {reason}`, "errors.duel_cannot_save_to_cache": `Cannot save duel data to cache`, "errors.duel_cannot_get_with_settings": `Cannot get duel channel settings`, "errors.8ball_cannot_find": `Cannot find 8ball settings`, "info.duel_stats": `You have shoot {duels} times · {wins} W – {loses} L ({winrate} WR)`, "info.sender_already_in_duel": `You already in duel`, "errors.duel_cannot_get_sender": `Cannot get sender current duel`, "errors.roulette_cannot_get_with_settings": `Cannot get roulette settings from database`, "errors.duel_with_yourself": `You cannot duel with yourself`, "errors.duel_cannot_set_user_cooldown": `Cannot set user cooldown: {reason}`, "info.voteban_in_progress": `Another voteban in progress`, "info.user_already_in_duel": `Target user already in duel`, "errors.duel_cannot_save_result": `Cannot save duel result: {reason}`, "errors.voteban_cannot_check_progress": `Cannot check if vote in progress`, "errors.seppuku_cannot_find_settings": `Cannot find seppuku settings`}, "marker": map[string]string{"errors.cannot_create_marker": `Cannot create marker. {reason}`, "success.marker_created": `Marker created`}, "predictions": map[string]string{"errors.cannot_cancel": `Cannot cancel prediction`, "info.cancel": `✅ Prediction canceled`, "info.no_runed": `No prediction runed`, "hints.startPredictionArgTitle": `Will we win this game?`, "errors.cannot_get_current_var": `Cannot get current prediction: {reason}`, "errors.cannot_cancel_var": `Cannot cancel prediction: {reason}`, "errors.no_variant": `No prediction variant`, "info.locked": `✅ Prediction locked`, "info.started": `✅ Prediction started`, "hints.startPredictionDuration": `120`, "hints.startPredictionArgVariants": `Yes, win / No, lose`, "info.resolved": `✅ Prediction resolved`, "hints.predictionResolveOutcomeNum": `Variant number, for example: 1,2,3,4,5`, "errors.cannot_create": `Cannot create prediction`, "errors.cannot_create_var": `Cannot create prediction: {reason}`, "errors.cannot_get_current": `Cannot get current prediction`}, "prefix": map[string]string{"errors.cannot_update": `Cannot update prefix`, "success.updated": `Prefix updated`, "errors.required": `Prefix is required`, "errors.too_long": `Prefix cannot be longer than 10 characters`, "errors.cannot_get_current": `Cannot get current prefix`, "errors.cannot_create": `Cannot create prefix`}, "dudes": map[string]string{"info.color_reset": `Color reset to default`, "errors.sprite_invalid": `Invalid sprite, available: {availableSprites}`, "errors.color_cannot_trigger": `Cannot trigger dudes color`, "errors.jump_cannot_trigger": `Cannot trigger dudes jump`, "info.color_changed": `Color changed to {dudeColor}`, "info.sprite_changed": `Sprite changed to {dudeSprite}`, "info.color": `Your color is {dudeColor}`, "info.sprite": `Your sprite is {dudeSprite}`, "errors.leave_cannot_trigger": `Cannot trigger dudes leave`, "errors.sprite_cannot_trigger": `Cannot trigger dudes sprite`, "errors.color_invalid": `Invalid color`, "errors.grow_cannot_trigger": `Cannot trigger dudes grow`, "info.color_required": `Color is required`, "info.sprite_required": `Sprite is required, available: {availableSprites}`}, "permit": map[string]string{"errors.cannot_create": `Cannot create permit`, "success.added_permit": `✅ Added {countPermit} permits to {userName}`}, "subage": map[string]string{"errors.not_subscriber_or_hidden": `User is not a subscriber or their subscription info is hidden`, "responses.streak_info": `(streak: {months} months)`, "responses.time_remaining": `(time remaining: {duration})`, "responses.not_subscriber": `{user} is not a subscriber`, "responses.not_subscriber_but_was": `{user} is not a subscriber but was subscribed for {months} months`, "responses.subscription_info": `{user} is a {tier} subscriber of {channel} for {months} months`}, "vips": map[string]string{"hints.unvip_in": `Time in, example: 1w5d1m5s`, "errors.updated": `✅ Updated vip for user {userName} new expriation time {endTime}`, "errors.already_have_role": `User already vip or moderator!`, "errors.cannot_get_list_from_db": `Cannot get vip list from database`, "errors.cannot_create_scheduled_in_db": `Cannot create scheduled vip in database`, "errors.added_with_remove_time": `✅ Added vip to {userName}, will be removed at {endTime}`, "errors.cannot_update": `Cannot update scheduled vip`, "errors.removed": `✅ Removed vip from {userName}`, "errors.no_scheduled_vips": `There are no scheduled vips`, "errors.invalid_duration": `Invalid duration format. Please use formats like <1h>, <30m>, or <2d>`, "errors.added": `✅ Added vip to {userName}`, "hints.user": `@username`}, "manage": map[string]string{"errors.command_cannot_update_response": `Cannot update response because you have more than 1 response in command. Please use UI.`, "add.alias_add": `✅ Alias added`, "errors.alias_already_exist": `Command with {alias} name or alias already exists`, "errors.command_large_size": `Command name cannot be greatest then 20.`, "errors.command_cannot_update": `Cannot update command`, "errors.command_with_name_cannot_find": `Command with that name not found`, "errors.command_cannot_delete_default": `Cannot delete default command`, "add.command_add": `✅ Command added`, "edit.command_edited": `✅ Command edited`, "remove.command_removed": `✅ Command removed`, "errors.command_with_alias_already_exists": `Command with that name or alias already exists`, "errors.command_cannot_get": `Cannot get command`, "errors.alias_not_command": `That alias not in the command`, "errors.command_have_no_aliases": `Command have no aliases`, "errors.command_cannot_save": `Cannot save command`, "remove.alias_removed": `✅ Alias removed`, "errors.alias_cannot_get_existed_commands": `Cannot get existed commands`, "errors.alias_cannot_update": `Cannot update command aliases`, "errors.command_not_found": `Command not found`}}, "errors": map[string]map[string]string{"generic": map[string]string{"twir_error": `[Twir error]: Unknown error happened. Please contact developers.`, "cannot_timeout_user": `Cannot timeout user`, "cannot_get_user": `Cannot get user: {reason}`, "getting_channel_settings": `Error while getting channel settings`, "should_mention_with_at": `You should tag user with @`, "cannot_get_message": `Cannot get messages`, "cannot_find_user_twitch": `Cannot find user on twitch`, "cannot_find_users_twitch": `Cannot find users on twitch`, "cannot_get_accept_command_name": `Cannot get accept command name`, "internal": `❌ Internal error`, "user_not_found": `User not found`, "something_went_wrong": `Something went wrong, please try again later`, "getting_user_settings": `Error while getting user settings`, "cannot_find_channel_db": `Cannot find channel in database`, "cannot_get_db_channel": `Cannot get database channel`, "cannot_find_user_db": `Cannot find user in database`, "broadcaster_client": `Cannot create broadcaster twitch client`, "cannot_find_channel_twitch": `Cannot find channel on twitch. {reason}`, "cannot_get_command": `Cannot get command`, "updating_settings": `Error while updating settings`, "cannot_get_moderators": `Cannot get moderators: {reason}`, "cannot_create_twitch": `Cannot create twitch client`, "cannot_ban_user": `Cannot ban user`, "create_settings": `Error while creating settings`, "cannot_get_stream": `Cannot get stream. {reason}`, "cannot_create_command": `Cannot create command`}}, "services": map[string]map[string]string{"chat_wall": map[string]string{"errors.create_chat_wall": `Cannot create chat wall: {reason}`, "errors.create_chat_logs_in_db": `Cannot create chat wall logs in db: {reason}`, "errors.get_chat_walls": `Cannot get chat walls: {reason}`, "errors.get_current_chat_walls": `Cannot get current chat walls: {reason}`, "errors.update_chat_walls": `Cannot update chat wall: {reason}`, "errors.get_users_stats": `Cannot get users stats: {reason}`, "info.banned_by_twir": `banned by twir for chat wall phrase: {banPhrase}`, "errors.get_already_handled": `Cannot get already handled messages: {reason}`, "errors.create_chat_wall_with_phrase": `Cannot create chat wall with phrase that already exists`, "errors.chat_wall_not_found": `Chat wall not found`, "errors.get_chat_wall_settings": `Cannot get chat wall settings: {reason}`, "errors.publish_deleted_messages": `Cannot publish deleted messages: {reason}`, "errors.handled_messages_to_redis": `Cannot add handled messages to redis: {reason}`, "errors.publish_ban_users": `Cannot publish ban users: {reason}`}, "shortenedurls": map[string]string{"errors.invalid_url": `Invalid url`}, "tts": map[string]string{"errors.voice_disallowed": `Voice {userVoice} is disallowed for usage`, "info.not_configured": `TTS not configured`, "info.no_voices": `No voices available`, "errors.not_found": `Voice {userVoice} not found`}}, "variables": map[string]map[string]string{"7tv": map[string]string{"errors.no_active_set": `You don't have a 7TV set set`, "errors.profile_not_found": `Failed to get 7tv profile: {reason}`, "errors.emote_not_found": `Emote {name} not found`, "errors.no_paint": `No paint`, "errors.no_roles": `No roles`}}}}
