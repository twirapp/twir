...
type KeysVariablesUserInfoHoursVars struct {
	Hours any
}
type KeysVariablesUserInfoHours struct {
	Vars twiri18n.Vars
}

func (k KeysVariablesUserInfoHours) IsTranslationKey() {
}
func (k KeysVariablesUserInfoHours) GetPath() string {
	return "variables.user.info.hours"
}
func (k KeysVariablesUserInfoHours) GetPathSlice() []string {
	return []string{"variables", "user", "info", "hours"}
}
func (k KeysVariablesUserInfoHours) GetVars() twiri18n.Vars {
	return k.Vars
}
func (k KeysVariablesUserInfoHours) SetVars(vars KeysVariablesUserInfoHoursVars) twiri18n.TranslationKey[KeysVariablesUserInfoHoursVars] {
	k.Vars = twiri18n.Vars{"hours": vars.Hours}
	return k
}

type KeysVariablesUserInfo struct {
	Hours KeysVariablesUserInfoHours
}
type KeysVariablesUser struct {
	Errors	KeysVariablesUserErrors
	Info	KeysVariablesUserInfo
}
...
var Store twiri18n.LocalesStore = twiri18n.LocalesStore{"en": map[string]map[string]map[string]string{"commands": map[string]map[string]string{"7tv": map[string]string{"errors.emote_already_exist_in_channel": `❌ Emote {emoteName} already exists in this channel`, "errors.emote_not_found_in_emoteset": `Emote {emoteName} not found in set {emoteSet}`, "errors.emoteset_not_active": `You don't have an active 7TV emote set`, "profile_info.response": `{profileName} · Paint: {paintName} ({unlockedPaints} unlocked) · Roles: {roles} · Editor for {editorCount} · Set: {emoteSetName} ({emoteSetCount}/{emoteSetCapacity}) · Created: {profileCreatedAt}`, "hints.copySetNameOfSet": `Name of set to copy`, "errors.emote_failed_to_remove": `Failed to remove 7tv emote: {reason}`, "errors.emote_failed_to_add": `Failed to add 7tv emote: {reason}`, "errors.emote_failed_to_fetch": `Failed to fetch 7tv emote: {reason}`, "errors.emote_failed_to_rename": `Failed to rename 7tv emote: {reason}`, "emote_info.response": `{name}: {link} · Added by @{addedByUserName} {addedByTime} ago · Author {emoteAuthor}`, "hints.emoteForCopyAlias": `Alias for emote`, "hints.emoteForCopyArgName": `Name of emote to copy`, "errors.profile_not_found": `7tv profile not found`, "errors.emote_not_found": `Emote {emoteName} not found.`, "errors.emoteset_broadcaster_not_active": `❌ No active emote set for broadcaster`, "errors.emote_not_found_in_channel": `❌ Emote {emoteSearch} not found in target channel`, "errors.emoteset_not_found": `Emote set {emoteName} not found`, "rename.emote_rename": `✅ Emote {oldEmoteName} renamed to {newEmoteName}"`, "hints.emoteForAddArgLink": `Link or name`, "hints.emoteForAddArgAlias": `Optional alias`, "errors.profile_failed_to_get": `Failed to get 7tv profile: {reason}`, "add.emote_add": `✅ Emote added`, "remove.emote_remove": `✅ Emote {emoteName} removed`, "hints.copySetChannelName": `@channelName`}, "channel": map[string]string{"errors.broadcaster_twitch_api_client": `Cannot create broadcaster twitch api client: {reason}`, "errors.channel_cannot_get_information": `Cannot get channel information`, "errors.history_game_message": `Cannot find used games in database`, "hints.gameArgName": `Category name or created category alias`, "errors.history_title_message": `Cannot get history of titles from database: {reason}`, "errors.category_cannot_change_error": `Cannot change category: {errorMessage}`, "errors.game_not_found": `Game not found on twitch`, "errors.channel_not_found": `Channel not found`, "errors.category_cannot_get": `Cannot get category`, "errors.category_not_found": `Category not found`, "errors.alias_cannot_get_category": `Cannot get category aliases`, "add.category_change": `✅ {categoryName}`, "errors.broadcaster_twitch_client_cannot_create": `Cannot create broadcaster twitch client`, "errors.category_cannot_get_error": `Cannot get category: {errorMessage}`, "errors.category_cannot_change": `Cannot change category`}, "games": map[string]string{"errors.voteban_cannot_find_settings": `Cannot find voteban settings`, "errors.duel_with_bot": `You cannot duel with bot`, "errors.duel_cannot_check_cooldown": `Cannot check duel cooldown: {reason}`, "errors.roulette_cannot_send_initial_message": `Cannot send initial message`, "errors.duel_cannot_check_user": `Cannot check user in duel: {reason}`, "errors.voteban_cannot_check_progress": `Cannot check if vote in progress`, "errors.duel_cannot_set_user_cooldown": `Cannot set user cooldown: {reason}`, "errors.duel_cannot_save_data": `Cannot save duel data: {reason}`, "info.duel_stats": `You have shoot {duels} times · {wins} W – {loses} L ({winrate} WR)`, "errors.voteban_cannot_set_vote_expiration": `Cannot set vote expiration`, "errors.8ball_cannot_find": `Cannot find 8ball settings`, "errors.duel_cannot_set_global_cooldown": `Cannot set global cooldown: {reason}`, "info.sender_already_in_duel": `You already in duel`, "info.voteban_in_progress": `Another voteban in progress`, "errors.voteban_cannot_set_vote": `Cannot set vote`, "info.user_not_participate": `You are not participate in any duel`, "errors.duel_cannot_save_to_cache": `Cannot save duel data to cache`, "errors.duel_cannot_get_sender": `Cannot get sender current duel`, "errors.duel_cannot_save_result": `Cannot save duel result: {reason}`, "errors.voteban_cannot_lock": `Cannot lock voteban`, "errors.duel_cannot_get_with_settings": `Cannot get duel channel settings`, "errors.roulette_cannot_send_death_message": `Cannot send death message`, "errors.duel_with_yourself": `You cannot duel with yourself`, "errors.duel_cannot_validate_participants": `Cannot validate participants`, "errors.duel_with_streamer": `You cannot duel with streamer`, "info.user_already_in_duel": `Target user already in duel`, "errors.voteban_cannot_find_user": `Cannot find target user`, "errors.roulette_cannot_get_with_settings": `Cannot get roulette settings from database`, "errors.seppuku_cannot_find_settings": `Cannot find seppuku settings`}, "songrequest": map[string]string{"errors.get_latest_song": `Cannot get latest song`, "validate.errors.internal_error": `Internal error when checking follow`, "validate.errors.need_follow": `For request song you need to be a followed`, "errors.get_songs_from_queue": `Cannot get songs from queue`, "errors.update_song": `Cannot update song`, "errors.remove_song_from_queue": `Cannot remove song from queue`, "errors.get_current_song": `Cannot get current song`, "info.song_skipped": `Song {songTitle} skipped`, "info.delete": `Song {songTitle} deleted from queue`, "errors.search_song": `Cannot search song`, "errors.get_users_count": `Cannot get online users count`, "errors.get_settings": `Cannot get song requests settings`, "errors.get_current_vote": `Cannot get current vote`, "info.no_requested_songs": `You haven't requested any song`, "validate.errors.restrictions_on_user": `There are restrictions on user, but i cannot find you in db, sorry :(`, "errors.not_found": `Current song not found`, "errors.get_current_queue_count": `Cannot get current queue count`, "errors.get_votes_count": `Cannot get votes count`, "info.only_count_songs": `There is only {songsCount} songs`}, "stats": map[string]string{"info.watching_stream": `You watching stream for {userWatching}`, "info.watched": `{userWatched} watched`, "info.messages": `{userMessages} messages`, "info.emotes": `{userEmotes} used emotes`, "info.points": `{userPoints} used points`, "info.songs": `{userSongs} songs requestes`}, "nuke": map[string]string{"errors.timeout_duration": `Duration of timeout cannot be longer than 2 weeks`, "errors.parse_duration": `Cannot parse duration`, "hints.nukeTimeArgName": `time, examples: 10m, 10, 1h5m`, "errors.invalid_duration": `Invalid duration. Examples: !nuke 10m phrase, !nuke 10 phrase, !nuke 1h5m phrase`, "errors.cannot_get_users_stats": `Cannot get users stats`, "errors.cannot_get_handeled_messages": `Cannot get handled messages`, "errors.cannot_delete_messages": `Cannot delete messages`}, "permit": map[string]string{"errors.cannot_create": `Cannot create permit`, "success.added_permit": `✅ Added {countPermit} permits to {userName}`}, "shorturl": map[string]string{"errors.cannot_create_short_url": `Cannot create short url. {error}`, "success.short_url_created": `Short url: {url}`}, "vips": map[string]string{"hints.unvip_in": `Time in, example: 1w5d1m5s`, "errors.invalid_duration": `Invalid duration format. Please use formats like <1h>, <30m>, or <2d>`, "errors.cannot_create_scheduled_in_db": `Cannot create scheduled vip in database`, "errors.updated": `✅ Updated vip for user {userName} new expriation time {endTime}`, "errors.added": `✅ Added vip to {userName}`, "errors.no_scheduled_vips": `There are no scheduled vips`, "errors.removed": `✅ Removed vip from {userName}`, "hints.user": `@username`, "errors.already_have_role": `User already vip or moderator!`, "errors.cannot_get_list_from_db": `Cannot get vip list from database`, "errors.cannot_update": `Cannot update scheduled vip`, "errors.added_with_remove_time": `✅ Added vip to {userName}, will be removed at {endTime}`}, "chat_wall": map[string]string{"hints.timeoutDurationArgName": `Time. Examples: 10m, 10, 1h5m`, "errors.invalid_duration": `Invalid duration. Cannot be longer 2w Examples: 10m, 10, 1h5m`, "start.chat_wall_start": `✅ Chat wall started for 10 minutes, you can stop it with !chat wall stop {chatWallPhrase}`, "stop.chat_wal_stop": `✅ Chat wall {chatWallPhrase} stopped`, "hints.banPhraseArgName": `Phrase to ban`, "hints.deletePhraseArgName": `Phrase to delete`, "errors.chat_wall_not_found": `Chat wall {errorPhrase} not found or already stopped`, "errors.long_duration_timeout": `Duration of timeout cannot be longer than 2 weeks`, "errors.duration_cannot_parse": `Cannot parse duration`, "hints.timeoutPhraseArgName": `Phrase to ban`}, "prefix": map[string]string{"success.updated": `Prefix updated`, "errors.required": `Prefix is required`, "errors.too_long": `Prefix cannot be longer than 10 characters`, "errors.cannot_get_current": `Cannot get current prefix`, "errors.cannot_create": `Cannot create prefix`, "errors.cannot_update": `Cannot update prefix`}, "shoutout": map[string]string{"errors.bot_have_no_permissions": `We have no permissions for shoutout. Streamer must re-authorize to bot dashboard.`, "response_online": `Join https://twitch.tv/{userName} shining in 🎮 {categoryName} 📜 {title} with 👁️ {viewers} viewers!`, "response_offline": `Explore https://twitch.tv/{userName}’s world, last featured in 🎮 {categoryName} 📜 {title}!`}, "tts": map[string]string{"errors.not_configured": `TTS is not configured for this channel`, "errors.sending_to_tts": `Error while sending message to tts service`, "errors.while_getting_voices": `Error while getting voices`, "info.change_volume": `TTS volume changed to {userVolume}`, "info.rate": `Global rate: {globalRate} | Your rate: {userRate}`, "info.change_pitch": `Pitch changed to {newPitch}`, "info.voice": `Global voice: {globalVoice} | Your voice: {userVoice}`, "info.current_volume": `Current volume: {ttsVolume}`, "info.disabled": `TTS disabled`, "errors.while_disable": `Error while disabling tts`, "errors.while_enable": `Error while enabling tts`, "info.no_voices": `No voices available`, "info.voice_disallowed": `Voice {voiceName} is disallowed for usage`, "info.enabled": `TTS enabled`, "info.pitch": `Global pitch: {globalPitch} | Your pitch: {userPitch}`, "info.change_voice": `Voice changed to {newVoice}`, "info.change_rate": `Rate changed to {newRate}`}, "categories_aliases": map[string]string{"errors.category_failed_to_get": `Failed to get caterogies`, "errors.category_failed_to_create": `Failed to create category`, "errors.category_cannot_delete": `Cannot delete category`, "errors.alias_already_exists": `Alias {aliasName} already exists`, "errors.alias_not_found": `Category alias not found`, "errors.twitch_client_cannot_to_create": `Cannot create twitch client`, "errors.category_cannot_to_get": `Cannot get categories`, "errors.category_required": `Alias and category are required`, "errors.alias_empty": `No categories aliases created`, "errors.alias_removed": `Category alias {aliasName} removed`, "errors.category_not_found": `Category not found`, "add.alias_add_to_category": `Category alias {aliasName} added with category {categoryName}`, "errors.game_cannot_to_get": `Cannot get games`}, "clip": map[string]string{"cannot_get_clip": `Cannot get created clip`, "clip_created": `Clip created: {url}`, "cannot_create_clip": `Cannot create clip`, "empty_clip_url": `Clip URL is empty, please try again`}, "dudes": map[string]string{"info.sprite_required": `Sprite is required, available: {availableSprites}`, "info.color_reset": `Color reset to default`, "errors.color_invalid": `Invalid color`, "errors.sprite_invalid": `Invalid sprite, available: {availableSprites}`, "errors.jump_cannot_trigger": `Cannot trigger dudes jump`, "info.sprite_changed": `Sprite changed to {dudeSprite}`, "info.color_required": `Color is required`, "errors.color_cannot_trigger": `Cannot trigger dudes color`, "errors.grow_cannot_trigger": `Cannot trigger dudes grow`, "errors.leave_cannot_trigger": `Cannot trigger dudes leave`, "errors.sprite_cannot_trigger": `Cannot trigger dudes sprite`, "info.color_changed": `Color changed to {dudeColor}`, "info.color": `Your color is {dudeColor}`, "info.sprite": `Your sprite is {dudeSprite}`}, "manage": map[string]string{"errors.alias_cannot_get_existed_commands": `Cannot get existed commands`, "errors.alias_already_exist": `Command with {alias} name or alias already exists`, "edit.command_edited": `✅ Command edited`, "errors.command_cannot_get": `Cannot get command`, "errors.command_cannot_update_response": `Cannot update response because you have more than 1 response in command. Please use UI.`, "remove.alias_removed": `✅ Alias removed`, "remove.command_removed": `✅ Command removed`, "errors.command_have_no_aliases": `Command have no aliases`, "errors.command_with_name_cannot_find": `Command with that name not found`, "errors.command_cannot_save": `Cannot save command`, "errors.command_cannot_delete_default": `Cannot delete default command`, "errors.command_not_found": `Command not found`, "errors.command_cannot_update": `Cannot update command`, "errors.alias_not_command": `That alias not in the command`, "add.command_add": `✅ Command added`, "add.alias_add": `✅ Alias added`, "errors.command_with_alias_already_exists": `Command with that name or alias already exists`, "errors.alias_cannot_update": `Cannot update command aliases`, "errors.command_large_size": `Command name cannot be greatest then 20.`}, "marker": map[string]string{"success.marker_created": `Marker created`, "errors.cannot_create_marker": `Cannot create marker. {reason}`}, "predictions": map[string]string{"hints.startPredictionArgTitle": `Will we win this game?`, "errors.cannot_cancel_var": `Cannot cancel prediction: {reason}`, "info.cancel": `✅ Prediction canceled`, "info.resolved": `✅ Prediction resolved`, "errors.cannot_get_current": `Cannot get current prediction`, "errors.cannot_get_current_var": `Cannot get current prediction: {reason}`, "errors.cannot_cancel": `Cannot cancel prediction`, "info.no_runed": `No prediction runed`, "hints.startPredictionArgVariants": `Yes, win / No, lose`, "hints.predictionResolveOutcomeNum": `Variant number, for example: 1,2,3,4,5`, "errors.cannot_create_var": `Cannot create prediction: {reason}`, "errors.cannot_create": `Cannot create prediction`, "info.started": `✅ Prediction started`, "hints.startPredictionDuration": `120`, "errors.no_variant": `No prediction variant`, "info.locked": `✅ Prediction locked`}, "subage": map[string]string{"responses.streak_info": `(streak: {months} months)`, "responses.time_remaining": `(time remaining: {duration})`, "responses.not_subscriber": `{user} is not a subscriber`, "errors.not_subscriber_or_hidden": `User is not a subscriber or their subscription info is hidden`, "responses.not_subscriber_but_was": `{user} is not a subscriber but was subscribed for {months} months`, "responses.subscription_info": `{user} is a {tier} subscriber of {channel} for {months} months`}}, "errors": map[string]map[string]string{"generic": map[string]string{"cannot_find_channel_twitch": `Cannot find channel on twitch. {reason}`, "should_mention_with_at": `You should tag user with @`, "cannot_find_channel_db": `Cannot find channel in database`, "cannot_find_user_twitch": `Cannot find user on twitch`, "cannot_create_twitch": `Cannot create twitch client`, "broadcaster_client": `Cannot create broadcaster twitch client`, "getting_channel_settings": `Error while getting channel settings`, "cannot_ban_user": `Cannot ban user`, "cannot_get_db_channel": `Cannot get database channel`, "cannot_get_stream": `Cannot get stream. {reason}`, "cannot_get_moderators": `Cannot get moderators: {reason}`, "getting_user_settings": `Error while getting user settings`, "not_a_follower": `Not a follower`, "cannot_get_command": `Cannot get command`, "twir_error": `[Twir error]: Unknown error happened. Please contact developers.`, "cannot_find_user_db": `Cannot find user in database`, "cannot_find_users_twitch": `Cannot find users on twitch`, "cannot_get_message": `Cannot get messages`, "cannot_timeout_user": `Cannot timeout user`, "updating_settings": `Error while updating settings`, "create_settings": `Error while creating settings`, "cannot_create_command": `Cannot create command`, "cannot_get_user": `Cannot get user: {reason}`, "user_not_found": `User not found`, "cannot_get_accept_command_name": `Cannot get accept command name`, "something_went_wrong": `Something went wrong, please try again later`, "internal": `❌ Internal error`}}, "services": map[string]map[string]string{"shortenedurls": map[string]string{"errors.invalid_url": `Invalid url`}, "tts": map[string]string{"errors.not_found": `Voice {userVoice} not found`, "errors.voice_disallowed": `Voice {userVoice} is disallowed for usage`, "info.not_configured": `TTS not configured`, "info.no_voices": `No voices available`}, "chat_wall": map[string]string{"errors.create_chat_wall_with_phrase": `Cannot create chat wall with phrase that already exists`, "errors.create_chat_wall": `Cannot create chat wall: {reason}`, "errors.update_chat_walls": `Cannot update chat wall: {reason}`, "errors.get_current_chat_walls": `Cannot get current chat walls: {reason}`, "errors.publish_deleted_messages": `Cannot publish deleted messages: {reason}`, "errors.chat_wall_not_found": `Chat wall not found`, "errors.get_chat_walls": `Cannot get chat walls: {reason}`, "info.banned_by_twir": `Banned by twir for chat wall phrase: {banPhrase}`, "errors.get_chat_wall_settings": `Cannot get chat wall settings: {reason}`, "errors.get_already_handled": `Cannot get already handled messages: {reason}`, "errors.get_users_stats": `Cannot get users stats: {reason}`, "errors.publish_ban_users": `Cannot publish ban users: {reason}`, "errors.create_chat_logs_in_db": `Cannot create chat wall logs in db: {reason}`, "errors.handled_messages_to_redis": `Cannot add handled messages to redis: {reason}`}}, "variables": map[string]map[string]string{"chat_eval": map[string]string{"info.wrong_code": `Probably you're doing some suspicious things or wrote wrong code.`}, "custom_var": map[string]string{"errors.update_custom_var": `Cannot update custom variables`, "errors.evaluate_variable": `Cannot evaluate variable. This is internal error, please report this bug`, "errors.wrong_numbers": `Wrong number: {reason}`}, "followers": map[string]string{"errors.get_followers": `Cannot get followers`}, "keywords": map[string]string{"errors.id_not_provided": `ID is not provided`, "errors.not_found": `Keyword not found`}, "request": map[string]string{"errors.execute_request": `Cannot execute request`}, "stream": map[string]string{"info.no_history": `No history recorded`, "errors.offline": `offline`, "errors.count_followers": `Cannot count followers`, "errors.get_history_of_categories": `Cannot get history of categories`, "errors.error": `Error`, "info.offline": `Offline or error on getting category`}, "valorant": map[string]string{"info.matches": `{matchResult}({roundsWon}/{roundsLost}}) — {char} {KDA}`}, "countdown": map[string]string{"errors.not_passed_params": `Have not passed params to variable`, "errors.parse_date": `Cannot parse date`}, "random": map[string]string{"errors.parse_first_number": `Cannot parse first number from arguments`, "errors.lower_numbers": `Numbers cannot be lower then 0`, "errors.not_passed_params": `Have not passed params to random variable`, "errors.parse_second_number": `Cannot parse second number from arguments`, "errors.parameters_not_specified": `Parameters are not specified`, "errors.wrong_with_params": `Something is wrong with your params`, "errors.wrong_number": `Wrong number of arguments passed to random`, "errors.first_larger_second": `First number cannot be larger then second`, "errors.empty_phrase": `Your phrases contains empty phrase, check you writed commas correctly`, "errors.get_online_user": `Cannot get online user`}, "user": map[string]string{"errors.find_user_on_twitch": `Cannot find user on twitch`, "info.hours": `{hours}h`}, "7tv": map[string]string{"errors.emote_not_found": `Emote {name} not found`, "errors.no_paint": `No paint`, "errors.no_roles": `No roles`, "errors.no_active_set": `You don't have a 7TV set set`, "errors.profile_not_found": `Failed to get 7tv profile: {reason}`}, "commands": map[string]string{"info.no_passed_params": `Have not passed params to variable`, "info.command_with_name_not_found": `Command with name {commandName} not found`, "info.get_count": `Cannot get count`}, "shorturl": map[string]string{"errors.url_required": `URL is required`, "errors.create_short_url": `Cannot create short url: {reason}`}, "song": map[string]string{"info.lastfm_integration": `LastFM integration not enabled`, "info.get_spotify_entity": `Failed to get spotify entity`, "info.get_spotify_integration": `Cannot get spotify integration: {reason}`, "errors.get_recent_tracks": `Cannot get recent tracks: {reason}`, "errors.create_lastfm_service": `Cannot create lastfm service: {reason}`, "errors.fetch_tracks_spotify": `Cannot fetch tracks from spotify: {reason}`, "errors.parse_played_at": `Cannot parse played at`, "info.no_needed_scope": `No needed scope, reconnect spotify in dashboard`, "info.spotify_not_connected": `Spotify not connected`, "info.failed_get_spotify_integration": `Failed to get spotify integration`, "info.history": `{trackTitle} - {trackArtist} (~{minutes}m ago)`, "errors.fetch_tracks_lastfm": `Cannot fetch tracks from lastfm: {reason}`, "info.no_integrations": `No integrations connected`}, "subscribers": map[string]string{"errors.get_subscribers": `Cannot get subscribers`}}}}
