// Code generated by swaggo/swag. DO NOT EDIT.

package docs

import "github.com/swaggo/swag"

const docTemplate = `{
    "schemes": {{ marshal .Schemes }},
    "swagger": "2.0",
    "info": {
        "description": "{{escape .Description}}",
        "title": "{{.Title}}",
        "contact": {},
        "version": "{{.Version}}"
    },
    "host": "{{.Host}}",
    "basePath": "{{.BasePath}}",
    "paths": {
        "/v1/channels/{channelId}/bot/checkmod": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Bot"
                ],
                "summary": "Check does bot moderator on the channel",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/bot.BotInfo"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/bot/connection": {
            "patch": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Bot"
                ],
                "summary": "Check does bot moderator on the channel",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_bot.connectionDto"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "boolean"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/categories-aliases": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "CategoriesAliases"
                ],
                "summary": "Get channel categories aliases list",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/model.ChannelCategoryAlias"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "CategoriesAliases"
                ],
                "summary": "Create category alias",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_categories-aliases.categoryAliasDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelCategoryAlias"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/categories-aliases/{categoryAliasId}": {
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "CategoriesAliases"
                ],
                "summary": "Delete category alias",
                "parameters": [
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of category",
                        "name": "categoryAliasId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    }
                }
            },
            "patch": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "CategoriesAliases"
                ],
                "summary": "Update category alias",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_categories-aliases.categoryAliasDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of category",
                        "name": "categoryAliasId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/commands": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Commands"
                ],
                "summary": "Get channel commands list",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/model.ChannelsCommands"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Commands"
                ],
                "summary": "Create command",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_commands.commandDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsCommands"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/commands/groups": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "CommandsGroup"
                ],
                "summary": "Get channel commands groups list",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/model.ChannelCommandGroup"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "CommandsGroup"
                ],
                "summary": "Create command group",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_commands_group.groupDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelCommandGroup"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/commands/groups/{groupId}": {
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "CommandsGroup"
                ],
                "summary": "Update command group",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_commands_group.groupDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of group",
                        "name": "groupId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelCommandGroup"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "CommandsGroup"
                ],
                "summary": "Delete command group",
                "parameters": [
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of group",
                        "name": "groupId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/commands/{commandId}": {
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Commands"
                ],
                "summary": "Update command",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_commands.commandDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of command",
                        "name": "commandId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsCommands"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Commands"
                ],
                "summary": "Delete command",
                "parameters": [
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of command",
                        "name": "commandId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsCommands"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "patch": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Commands"
                ],
                "summary": "Partialy update command",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_commands.commandPatchDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of command",
                        "name": "commandId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsCommands"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/community/users": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Community"
                ],
                "summary": "Get channel users list from database",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/internal_api_v1_community.User"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/community/users/stats": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Community"
                ],
                "summary": "Reset stats of channel",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/greetings": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Greetings"
                ],
                "summary": "Get channel greeting list",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/internal_api_v1_greetings.Greeting"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Greetings"
                ],
                "summary": "Create greeting",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_greetings.greetingsDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_greetings.Greeting"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/greetings/{greetingId}": {
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Greetings"
                ],
                "summary": "Update greeting",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_greetings.greetingsDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of greeting",
                        "name": "greetingId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_greetings.Greeting"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Greetings"
                ],
                "summary": "Delete greeting",
                "parameters": [
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of greeting",
                        "name": "greetingId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    }
                }
            },
            "patch": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Greetings"
                ],
                "summary": "Update greeting",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_greetings.greetingsPatchDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of greeting",
                        "name": "greetingId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_greetings.Greeting"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/donatepay": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "text/plain"
                ],
                "tags": [
                    "Integrations|DonatePay"
                ],
                "summary": "Get DonatePay token",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|DonatePay"
                ],
                "summary": "Authorize DonatePay",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_integrations_donatepay.createOrUpdateDTO"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/donationalerts": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|DonationAlerts"
                ],
                "summary": "Get DonationAlerts integration",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsIntegrationsData"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|DonationAlerts"
                ],
                "summary": "Update auth of DonationAlerts",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_integrations_donationalerts.tokenDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/donationalerts/auth": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "text/plain"
                ],
                "tags": [
                    "Integrations|DonationAlerts"
                ],
                "summary": "Get DonationAlerts auth link",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Auth link",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/donationalerts/logout": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|DonationAlerts"
                ],
                "summary": "Logout",
                "parameters": [
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiBadRequest"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/faceit": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Faceit"
                ],
                "summary": "Get Faceit profile",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsIntegrationsData"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Faceit"
                ],
                "summary": "Authorize Faceit",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_integrations_faceit.tokenDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/faceit/auth": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "text/plain"
                ],
                "tags": [
                    "Integrations|Faceit"
                ],
                "summary": "Get Faceit auth link",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Auth link",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/faceit/logout": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Faceit"
                ],
                "summary": "Logout",
                "parameters": [
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiBadRequest"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/lastfm": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Lastfm"
                ],
                "summary": "Get LastFm profile",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/internal_api_v1_integrations_lastfm.LastfmProfile"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Lastfm"
                ],
                "summary": "Update LastFM",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_integrations_lastfm.lastfmDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsIntegrations"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/lastfm/auth": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Lastfm"
                ],
                "summary": "Get LastFm auth link",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/lastfm/logout": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Lastfm"
                ],
                "summary": "Logout",
                "parameters": [
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiBadRequest"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/spotify": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Spotify"
                ],
                "summary": "Get Spotify profile",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/spotify.SpotifyProfile"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Spotify"
                ],
                "summary": "Authorize Spotify",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_integrations_spotify.tokenDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/spotify/auth": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "text/plain"
                ],
                "tags": [
                    "Integrations|Spotify"
                ],
                "summary": "Get Spotify auth link",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Auth link",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/spotify/logout": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Spotify"
                ],
                "summary": "Logout",
                "parameters": [
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiBadRequest"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/streamlabs": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Streamlabs"
                ],
                "summary": "Get Streamlabs integration",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsIntegrations"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Streamlabs"
                ],
                "summary": "Update auth of Streamlabs",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_integrations_streamlabs.tokenDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/streamlabs/auth": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "text/plain"
                ],
                "tags": [
                    "Integrations|Streamlabs"
                ],
                "summary": "Get DonationAlerts auth link",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Auth link",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/streamlabs/logout": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Streamlabs"
                ],
                "summary": "Logout",
                "parameters": [
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/valorant": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "text/plain"
                ],
                "tags": [
                    "Integrations|Valorant"
                ],
                "summary": "Get valorant data",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsIntegrationsData"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|Valorant"
                ],
                "summary": "Update valorant data",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_integrations_valorant.createOrUpdateDTO"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/vk": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|VK"
                ],
                "summary": "Get VK profile",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_integrations_vk.profile"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|VK"
                ],
                "summary": "Login via vk",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_integrations_vk.vkDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/vk/auth": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|VK"
                ],
                "summary": "Get VK auth link",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsIntegrations"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/integrations/vk/logout": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Integrations|VK"
                ],
                "summary": "Logout",
                "parameters": [
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/keywords": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Keywords"
                ],
                "summary": "Get channel keywords list",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/model.ChannelsKeywords"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Keywords"
                ],
                "summary": "Create keyword",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_keywords.keywordDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsKeywords"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/keywords/{keywordId}": {
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Keywords"
                ],
                "summary": "Update keyword",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_keywords.keywordDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of keyword",
                        "name": "keywordId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsKeywords"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Keywords"
                ],
                "summary": "Delete keyword",
                "parameters": [
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of keyword",
                        "name": "keywordId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsKeywords"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "patch": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Commands"
                ],
                "summary": "Partially update keyword",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_keywords.keywordPatchDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of keyword",
                        "name": "keywordId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsKeywords"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/moderation": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Moderation"
                ],
                "summary": "Get moderation settings",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/model.ChannelsModerationSettings"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Moderation"
                ],
                "summary": "Create command",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_moderation.moderationDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/model.ChannelsModerationSettings"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/moderation/category": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Moderation"
                ],
                "summary": "Change stream category",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_moderation.postCategoryDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/internal_api_v1_moderation.postCategoryResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/moderation/title": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Moderation"
                ],
                "summary": "Change stream title",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_moderation.postTitleDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/internal_api_v1_moderation.postTitleResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/modules/tts": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get TTS settings",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "TTS"
                ],
                "summary": "Get TTS settings",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Channel ID",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/modules.TTSSettings"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Set TTS settings",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "TTS"
                ],
                "summary": "Set TTS settings",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Channel ID",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "TTS settings",
                        "name": "settings",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/modules.TTSSettings"
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/modules/youtube-sr": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Modules|YouTube"
                ],
                "summary": "Get YouTube settings",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/modules.YouTubeSettings"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/modules/youtube-sr/search": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Modules|YouTube"
                ],
                "summary": "Search channel or video",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "Input string",
                        "name": "query",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "channel or video",
                        "name": "type",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/modules.SearchResult"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/rewards": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Rewards"
                ],
                "summary": "Get channel rewards list",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/helix.ChannelCustomReward"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/roles": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get roles",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Roles"
                ],
                "summary": "Get roles",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "Channel ID",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/model.ChannelRole"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Create role",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Roles"
                ],
                "summary": "Create role",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "Channel ID",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Role",
                        "name": "role",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_roles.roleDto"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelRole"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/roles/{roleId}": {
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update role",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Roles"
                ],
                "summary": "Update role",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "Channel ID",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "{{roleId}}",
                        "description": "Role ID",
                        "name": "roleId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Role",
                        "name": "role",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_roles.roleDto"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelRole"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiNotFoundError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Delete role",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Roles"
                ],
                "summary": "Delete role",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "Channel ID",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "{{roleId}}",
                        "description": "Role ID",
                        "name": "roleId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiNotFoundError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/roles/{roleId}/users": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Get users",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Roles"
                ],
                "summary": "Get users",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "Channel ID",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "{{roleId}}",
                        "description": "Role ID",
                        "name": "roleId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/internal_api_v1_roles_users.roleUser"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiNotFoundError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "Update users",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Roles"
                ],
                "summary": "Update users",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "Channel ID",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "default": "{{roleId}}",
                        "description": "Role ID",
                        "name": "roleId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Role",
                        "name": "role",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_roles_users.roleUserDto"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelRole"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiNotFoundError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/streams": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Streams"
                ],
                "summary": "Get channel stream",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsStreams"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/timers": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Timers"
                ],
                "summary": "Get channel timers list",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/model.ChannelsTimers"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Timers"
                ],
                "summary": "Create timer",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_timers.timerDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsTimers"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/timers/{timerId}": {
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Timers"
                ],
                "summary": "Update timer",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_timers.timerDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of timer",
                        "name": "timerId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsTimers"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Timers"
                ],
                "summary": "Delete timer",
                "parameters": [
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of timer",
                        "name": "timerId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "patch": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Timers"
                ],
                "summary": "Partially update timer",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_timers.timerPatchDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of timer",
                        "name": "timerId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsTimers"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/variables": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Variables"
                ],
                "summary": "Get channel variables list",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/model.ChannelsCustomvars"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Variables"
                ],
                "summary": "Create variable",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_variables.variableDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsCustomvars"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/variables/builtin": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Variables"
                ],
                "summary": "Get built-in variables",
                "parameters": [
                    {
                        "type": "string",
                        "default": "{{channelId}}",
                        "description": "ChannelId",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/parser.GetVariablesResponse_Variable"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/channels/{channelId}/variables/{variableId}": {
            "put": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Variables"
                ],
                "summary": "Update variable",
                "parameters": [
                    {
                        "description": "Data",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api_v1_variables.variableDto"
                        }
                    },
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of variable",
                        "name": "variableId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsCustomvars"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Variables"
                ],
                "summary": "Delete variable",
                "parameters": [
                    {
                        "type": "string",
                        "description": "ID of channel",
                        "name": "channelId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "ID of variable",
                        "name": "variableId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/model.ChannelsCustomvars"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError"
                        }
                    },
                    "404": {
                        "description": "Not Found"
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        },
        "/v1/stats": {
            "get": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Stats"
                ],
                "summary": "Get some bot statistic",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/internal_api_v1_stats.statsItem"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError"
                        }
                    }
                }
            }
        }
    },
    "definitions": {
        "bot.BotInfo": {
            "type": "object",
            "properties": {
                "botId": {
                    "type": "string"
                },
                "botName": {
                    "type": "string"
                },
                "enabled": {
                    "type": "boolean"
                },
                "isMod": {
                    "type": "boolean"
                }
            }
        },
        "github_com_satont_tsuwari_apps_api_internal_types.DOCApiBadRequest": {
            "type": "object",
            "properties": {
                "messages": {
                    "type": "string"
                }
            }
        },
        "github_com_satont_tsuwari_apps_api_internal_types.DOCApiInternalError": {
            "type": "object",
            "properties": {
                "messages": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "github_com_satont_tsuwari_apps_api_internal_types.DOCApiNotFoundError": {
            "type": "object",
            "properties": {
                "messages": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "github_com_satont_tsuwari_apps_api_internal_types.DOCApiValidationError": {
            "type": "object",
            "properties": {
                "messages": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "helix.ChannelCustomReward": {
            "type": "object",
            "properties": {
                "background_color": {
                    "type": "string"
                },
                "broadcaster_id": {
                    "type": "string"
                },
                "broadcaster_login": {
                    "type": "string"
                },
                "broadcaster_name": {
                    "type": "string"
                },
                "cooldown_expires_at": {
                    "type": "string"
                },
                "cost": {
                    "type": "integer"
                },
                "default_image": {
                    "$ref": "#/definitions/helix.RewardImage"
                },
                "global_cooldown_setting": {
                    "$ref": "#/definitions/helix.GlobalCooldownSettings"
                },
                "id": {
                    "type": "string"
                },
                "image": {
                    "$ref": "#/definitions/helix.RewardImage"
                },
                "is_enabled": {
                    "type": "boolean"
                },
                "is_in_stock": {
                    "type": "boolean"
                },
                "is_paused": {
                    "type": "boolean"
                },
                "is_user_input_required": {
                    "type": "boolean"
                },
                "max_per_stream_setting": {
                    "$ref": "#/definitions/helix.MaxPerStreamSettings"
                },
                "max_per_user_per_stream_setting": {
                    "$ref": "#/definitions/helix.MaxPerUserPerStreamSettings"
                },
                "prompt": {
                    "type": "string"
                },
                "redemptions_redeemed_current_stream": {
                    "type": "integer"
                },
                "should_redemptions_skip_request_queue": {
                    "type": "boolean"
                },
                "title": {
                    "type": "string"
                }
            }
        },
        "helix.GlobalCooldownSettings": {
            "type": "object",
            "properties": {
                "global_cooldown_seconds": {
                    "type": "integer"
                },
                "is_enabled": {
                    "type": "boolean"
                }
            }
        },
        "helix.MaxPerStreamSettings": {
            "type": "object",
            "properties": {
                "is_enabled": {
                    "type": "boolean"
                },
                "max_per_stream": {
                    "type": "integer"
                }
            }
        },
        "helix.MaxPerUserPerStreamSettings": {
            "type": "object",
            "properties": {
                "is_enabled": {
                    "type": "boolean"
                },
                "max_per_user_per_stream": {
                    "type": "integer"
                }
            }
        },
        "helix.RewardImage": {
            "type": "object",
            "properties": {
                "url_1x": {
                    "type": "string"
                },
                "url_2x": {
                    "type": "string"
                },
                "url_4x": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_bot.connectionDto": {
            "type": "object",
            "required": [
                "action"
            ],
            "properties": {
                "action": {
                    "type": "string",
                    "enum": [
                        "join",
                        "part"
                    ]
                }
            }
        },
        "internal_api_v1_categories-aliases.categoryAliasDto": {
            "type": "object",
            "required": [
                "alias",
                "category",
                "categoryId"
            ],
            "properties": {
                "alias": {
                    "type": "string",
                    "maxLength": 300,
                    "minLength": 1
                },
                "category": {
                    "type": "string",
                    "maxLength": 300,
                    "minLength": 1
                },
                "categoryId": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_commands.commandDto": {
            "type": "object",
            "required": [
                "aliases",
                "cooldownType",
                "enabled",
                "isReply",
                "keepResponsesOrder",
                "name",
                "onlineOnly",
                "visible"
            ],
            "properties": {
                "aliases": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "allowedUsersIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "cooldown": {
                    "type": "integer",
                    "maximum": 86400
                },
                "cooldownType": {
                    "type": "string"
                },
                "deniedUsersIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "description": {
                    "type": "string",
                    "maxLength": 500
                },
                "enabled": {
                    "type": "boolean"
                },
                "groupId": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "isReply": {
                    "type": "boolean"
                },
                "keepResponsesOrder": {
                    "type": "boolean"
                },
                "name": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 1
                },
                "onlineOnly": {
                    "type": "boolean"
                },
                "responses": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/internal_api_v1_commands.responsesDto"
                    }
                },
                "rolesIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "visible": {
                    "type": "boolean"
                }
            }
        },
        "internal_api_v1_commands.commandPatchDto": {
            "type": "object",
            "required": [
                "enabled"
            ],
            "properties": {
                "enabled": {
                    "type": "boolean"
                }
            }
        },
        "internal_api_v1_commands.responsesDto": {
            "type": "object",
            "properties": {
                "order": {
                    "type": "integer",
                    "minimum": 0
                },
                "text": {
                    "type": "string",
                    "maxLength": 500,
                    "minLength": 1
                }
            }
        },
        "internal_api_v1_commands_group.groupDto": {
            "type": "object",
            "required": [
                "color",
                "name"
            ],
            "properties": {
                "color": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_community.User": {
            "type": "object",
            "properties": {
                "avatarUrl": {
                    "type": "string"
                },
                "displayName": {
                    "type": "string"
                },
                "emotes": {
                    "type": "integer"
                },
                "id": {
                    "type": "string"
                },
                "messages": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "usedChannelPoints": {
                    "type": "string"
                },
                "watched": {
                    "type": "integer"
                }
            }
        },
        "internal_api_v1_greetings.Greeting": {
            "type": "object",
            "properties": {
                "avatar": {
                    "type": "string"
                },
                "channelId": {
                    "type": "string"
                },
                "enabled": {
                    "type": "boolean"
                },
                "id": {
                    "type": "string"
                },
                "isReply": {
                    "type": "boolean"
                },
                "processed": {
                    "type": "boolean"
                },
                "text": {
                    "type": "string"
                },
                "userId": {
                    "type": "string"
                },
                "userName": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_greetings.greetingsDto": {
            "type": "object",
            "required": [
                "enabled",
                "isReply",
                "userName"
            ],
            "properties": {
                "enabled": {
                    "type": "boolean"
                },
                "id": {
                    "type": "string"
                },
                "isReply": {
                    "type": "boolean"
                },
                "text": {
                    "type": "string",
                    "maxLength": 400
                },
                "userName": {
                    "type": "string",
                    "minLength": 1
                }
            }
        },
        "internal_api_v1_greetings.greetingsPatchDto": {
            "type": "object",
            "required": [
                "enabled"
            ],
            "properties": {
                "enabled": {
                    "type": "boolean"
                }
            }
        },
        "internal_api_v1_integrations_donatello.createOrUpdateDTO": {
            "type": "object",
            "properties": {
                "apiKey": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_integrations_donatepay.createOrUpdateDTO": {
            "type": "object",
            "properties": {
                "apiKey": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_integrations_donationalerts.tokenDto": {
            "type": "object",
            "required": [
                "code"
            ],
            "properties": {
                "code": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_integrations_faceit.tokenDto": {
            "type": "object",
            "required": [
                "code"
            ],
            "properties": {
                "code": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_integrations_lastfm.LastfmProfile": {
            "type": "object",
            "properties": {
                "albumCount": {
                    "type": "string"
                },
                "image": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "playCount": {
                    "type": "string"
                },
                "trackCount": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_integrations_lastfm.lastfmDto": {
            "type": "object",
            "required": [
                "code"
            ],
            "properties": {
                "code": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_integrations_spotify.tokenDto": {
            "type": "object",
            "required": [
                "code"
            ],
            "properties": {
                "code": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_integrations_streamlabs.tokenDto": {
            "type": "object",
            "required": [
                "code"
            ],
            "properties": {
                "code": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_integrations_valorant.createOrUpdateDTO": {
            "type": "object",
            "properties": {
                "username": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_integrations_vk.profile": {
            "type": "object",
            "properties": {
                "first_name": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "last_name": {
                    "type": "string"
                },
                "photo_max_orig": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_integrations_vk.vkDto": {
            "type": "object",
            "required": [
                "code"
            ],
            "properties": {
                "code": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_keywords.keywordDto": {
            "type": "object",
            "required": [
                "enabled",
                "isRegular",
                "isReply",
                "text",
                "usages"
            ],
            "properties": {
                "cooldown": {
                    "type": "integer",
                    "maximum": 86400,
                    "minimum": 5
                },
                "enabled": {
                    "type": "boolean"
                },
                "isRegular": {
                    "type": "boolean"
                },
                "isReply": {
                    "type": "boolean"
                },
                "response": {
                    "type": "string",
                    "maxLength": 500
                },
                "text": {
                    "type": "string",
                    "maxLength": 500
                },
                "usages": {
                    "type": "integer"
                }
            }
        },
        "internal_api_v1_keywords.keywordPatchDto": {
            "type": "object",
            "required": [
                "enabled"
            ],
            "properties": {
                "enabled": {
                    "type": "boolean"
                }
            }
        },
        "internal_api_v1_moderation.moderationDto": {
            "type": "object",
            "required": [
                "items"
            ],
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/internal_api_v1_moderation.moderationItemDto"
                    }
                }
            }
        },
        "internal_api_v1_moderation.moderationItemDto": {
            "type": "object",
            "required": [
                "banTime",
                "enabled",
                "id",
                "maxPercentage",
                "subscribers",
                "triggerLength",
                "type",
                "vips"
            ],
            "properties": {
                "banMessage": {
                    "type": "string"
                },
                "banTime": {
                    "type": "integer"
                },
                "blackListSentences": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "checkClips": {
                    "type": "boolean"
                },
                "enabled": {
                    "type": "boolean"
                },
                "id": {
                    "type": "string"
                },
                "maxPercentage": {
                    "type": "integer"
                },
                "subscribers": {
                    "type": "boolean"
                },
                "triggerLength": {
                    "type": "integer"
                },
                "type": {
                    "type": "string"
                },
                "vips": {
                    "type": "boolean"
                },
                "warningMessage": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_moderation.postCategoryDto": {
            "type": "object",
            "required": [
                "categoryId"
            ],
            "properties": {
                "categoryId": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_moderation.postCategoryResponse": {
            "type": "object",
            "properties": {
                "categoryId": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_moderation.postTitleDto": {
            "type": "object",
            "required": [
                "title"
            ],
            "properties": {
                "title": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_moderation.postTitleResponse": {
            "type": "object",
            "properties": {
                "title": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_roles.roleDto": {
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "name": {
                    "type": "string"
                },
                "permissions": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "users": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "internal_api_v1_roles_users.roleUser": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "string"
                },
                "userAvatar": {
                    "type": "string"
                },
                "userDisplayName": {
                    "type": "string"
                },
                "userId": {
                    "type": "string"
                },
                "userName": {
                    "type": "string"
                }
            }
        },
        "internal_api_v1_roles_users.roleUserDto": {
            "type": "object",
            "required": [
                "userNames"
            ],
            "properties": {
                "userNames": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "internal_api_v1_stats.statsItem": {
            "type": "object",
            "properties": {
                "count": {
                    "type": "integer"
                },
                "name": {
                    "type": "string",
                    "enum": [
                        "users",
                        "channels",
                        "commands",
                        "messages"
                    ]
                }
            }
        },
        "internal_api_v1_timers.responseDto": {
            "type": "object",
            "required": [
                "isAnnounce",
                "text"
            ],
            "properties": {
                "isAnnounce": {
                    "type": "boolean"
                },
                "text": {
                    "type": "string",
                    "maxLength": 400
                }
            }
        },
        "internal_api_v1_timers.timerDto": {
            "type": "object",
            "required": [
                "enabled",
                "name",
                "responses"
            ],
            "properties": {
                "enabled": {
                    "type": "boolean"
                },
                "messageInterval": {
                    "type": "integer",
                    "maximum": 10000,
                    "minimum": 0
                },
                "name": {
                    "type": "string",
                    "maxLength": 510
                },
                "responses": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/internal_api_v1_timers.responseDto"
                    }
                },
                "timeInterval": {
                    "type": "integer",
                    "maximum": 120,
                    "minimum": 1
                }
            }
        },
        "internal_api_v1_timers.timerPatchDto": {
            "type": "object",
            "required": [
                "enabled"
            ],
            "properties": {
                "enabled": {
                    "type": "boolean"
                }
            }
        },
        "internal_api_v1_variables.variableDto": {
            "type": "object",
            "required": [
                "name",
                "type"
            ],
            "properties": {
                "description": {
                    "type": "string"
                },
                "evalValue": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "response": {
                    "type": "string"
                },
                "type": {
                    "type": "string",
                    "enum": [
                        "SCRIPT",
                        "TEXT"
                    ]
                }
            }
        },
        "model.ChannelCategoryAlias": {
            "type": "object",
            "properties": {
                "alias": {
                    "type": "string"
                },
                "category": {
                    "type": "string"
                },
                "categoryId": {
                    "type": "string"
                },
                "channelId": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                }
            }
        },
        "model.ChannelCommandGroup": {
            "type": "object",
            "properties": {
                "channelId": {
                    "type": "string"
                },
                "color": {
                    "type": "string"
                },
                "commands": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/model.ChannelsCommands"
                    }
                },
                "id": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                }
            }
        },
        "model.ChannelRole": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "permissions": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "type": {
                    "$ref": "#/definitions/model.ChannelRoleEnum"
                }
            }
        },
        "model.ChannelRoleEnum": {
            "type": "string",
            "enum": [
                "BROADCASTER",
                "MODERATOR",
                "SUBSCRIBER",
                "VIP",
                "CUSTOM"
            ],
            "x-enum-varnames": [
                "ChannelRoleTypeBroadcaster",
                "ChannelRoleTypeModerator",
                "ChannelRoleTypeSubscriber",
                "ChannelRoleTypeVip",
                "ChannelRoleTypeCustom"
            ]
        },
        "model.Channels": {
            "type": "object",
            "properties": {
                "botId": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "isBanned": {
                    "type": "boolean"
                },
                "isBotMod": {
                    "type": "boolean"
                },
                "isEnabled": {
                    "type": "boolean"
                },
                "isTwitchBanned": {
                    "type": "boolean"
                }
            }
        },
        "model.ChannelsCommands": {
            "type": "object",
            "properties": {
                "aliases": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "allowedUsersIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "channelId": {
                    "type": "string"
                },
                "cooldown": {
                    "type": "integer"
                },
                "cooldownType": {
                    "type": "string"
                },
                "default": {
                    "type": "boolean"
                },
                "defaultName": {
                    "type": "string"
                },
                "deniedUsersIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "description": {
                    "type": "string"
                },
                "enabled": {
                    "type": "boolean"
                },
                "group": {
                    "$ref": "#/definitions/model.ChannelCommandGroup"
                },
                "groupId": {
                    "$ref": "#/definitions/null.String"
                },
                "id": {
                    "type": "string"
                },
                "isReply": {
                    "type": "boolean"
                },
                "keepResponsesOrder": {
                    "type": "boolean"
                },
                "module": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "onlineOnly": {
                    "type": "boolean"
                },
                "responses": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/model.ChannelsCommandsResponses"
                    }
                },
                "rolesIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "visible": {
                    "type": "boolean"
                }
            }
        },
        "model.ChannelsCommandsResponses": {
            "type": "object",
            "properties": {
                "commandId": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "order": {
                    "type": "integer"
                },
                "text": {
                    "type": "string"
                }
            }
        },
        "model.ChannelsCustomvars": {
            "type": "object",
            "properties": {
                "channelId": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "evalValue": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "response": {
                    "type": "string"
                },
                "type": {
                    "type": "string"
                }
            }
        },
        "model.ChannelsIntegrations": {
            "type": "object",
            "properties": {
                "accessToken": {
                    "type": "string"
                },
                "apiKey": {
                    "type": "string"
                },
                "channelId": {
                    "type": "string"
                },
                "clientId": {
                    "type": "string"
                },
                "clientSecret": {
                    "type": "string"
                },
                "data": {
                    "$ref": "#/definitions/model.ChannelsIntegrationsData"
                },
                "enabled": {
                    "type": "boolean"
                },
                "id": {
                    "type": "string"
                },
                "integrationId": {
                    "type": "string"
                },
                "refreshToken": {
                    "type": "string"
                }
            }
        },
        "model.ChannelsIntegrationsData": {
            "type": "object",
            "properties": {
                "avatar": {
                    "type": "string"
                },
                "code": {
                    "type": "string"
                },
                "game": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "userId": {
                    "type": "string"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "model.ChannelsKeywords": {
            "type": "object",
            "properties": {
                "channelId": {
                    "type": "string"
                },
                "cooldown": {
                    "type": "integer"
                },
                "cooldownExpireAt": {
                    "type": "string"
                },
                "enabled": {
                    "type": "boolean"
                },
                "id": {
                    "type": "string"
                },
                "isRegular": {
                    "type": "boolean"
                },
                "isReply": {
                    "type": "boolean"
                },
                "response": {
                    "type": "string"
                },
                "text": {
                    "type": "string"
                },
                "usages": {
                    "type": "integer"
                }
            }
        },
        "model.ChannelsModerationSettings": {
            "type": "object",
            "properties": {
                "banMessage": {
                    "type": "string"
                },
                "banTime": {
                    "type": "integer"
                },
                "blackListSentences": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "channelId": {
                    "type": "string"
                },
                "checkClips": {
                    "type": "boolean"
                },
                "enabled": {
                    "type": "boolean"
                },
                "id": {
                    "type": "string"
                },
                "maxPercentage": {
                    "type": "integer"
                },
                "subscribers": {
                    "type": "boolean"
                },
                "triggerLength": {
                    "type": "integer"
                },
                "type": {
                    "type": "string"
                },
                "vips": {
                    "type": "boolean"
                },
                "warningMessage": {
                    "type": "string"
                }
            }
        },
        "model.ChannelsStreams": {
            "type": "object",
            "properties": {
                "channel": {
                    "$ref": "#/definitions/model.Channels"
                },
                "communityIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "gameId": {
                    "type": "string"
                },
                "gameName": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "isMature": {
                    "type": "boolean"
                },
                "language": {
                    "type": "string"
                },
                "parsedMessages": {
                    "type": "integer"
                },
                "startedAt": {
                    "type": "string"
                },
                "tagIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "thumbnailUrl": {
                    "type": "string"
                },
                "title": {
                    "type": "string"
                },
                "type": {
                    "type": "string"
                },
                "userId": {
                    "type": "string"
                },
                "userLogin": {
                    "type": "string"
                },
                "userName": {
                    "type": "string"
                },
                "viewerCount": {
                    "type": "integer"
                }
            }
        },
        "model.ChannelsTimers": {
            "type": "object",
            "properties": {
                "enabled": {
                    "type": "boolean"
                },
                "id": {
                    "type": "string"
                },
                "messageInterval": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "responses": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/model.ChannelsTimersResponses"
                    }
                },
                "timeInterval": {
                    "type": "integer"
                }
            }
        },
        "model.ChannelsTimersResponses": {
            "type": "object",
            "properties": {
                "isAnnounce": {
                    "type": "boolean"
                },
                "text": {
                    "type": "string"
                }
            }
        },
        "modules.SearchResult": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "string"
                },
                "thumbNail": {
                    "type": "string"
                },
                "title": {
                    "type": "string"
                }
            }
        },
        "modules.TTSSettings": {
            "type": "object",
            "required": [
                "enabled",
                "voice"
            ],
            "properties": {
                "allow_users_choose_voice_in_main_command": {
                    "type": "boolean"
                },
                "disallowed_voices": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "do_not_read_emoji": {
                    "type": "boolean"
                },
                "do_not_read_links": {
                    "type": "boolean"
                },
                "do_not_read_twitch_emotes": {
                    "type": "boolean"
                },
                "enabled": {
                    "type": "boolean"
                },
                "max_symbols": {
                    "type": "integer"
                },
                "pitch": {
                    "type": "integer",
                    "maximum": 100,
                    "minimum": 0
                },
                "rate": {
                    "type": "integer",
                    "maximum": 100,
                    "minimum": 0
                },
                "read_chat_messages": {
                    "type": "boolean"
                },
                "read_chat_messages_nicknames": {
                    "type": "boolean"
                },
                "voice": {
                    "type": "string"
                },
                "volume": {
                    "type": "integer",
                    "maximum": 100,
                    "minimum": 0
                }
            }
        },
        "modules.YouTubeChannelTranslations": {
            "type": "object",
            "properties": {
                "denied": {
                    "type": "string"
                }
            }
        },
        "modules.YouTubeDenyList": {
            "type": "object",
            "required": [
                "artistsNames",
                "channels",
                "songs",
                "users"
            ],
            "properties": {
                "artistsNames": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "channels": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/modules.YouTubeDenySettingsChannels"
                    }
                },
                "songs": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/modules.YouTubeDenySettingsSongs"
                    }
                },
                "users": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/modules.YouTubeDenySettingsUsers"
                    }
                }
            }
        },
        "modules.YouTubeDenySettingsChannels": {
            "type": "object",
            "required": [
                "id",
                "thumbNail",
                "title"
            ],
            "properties": {
                "id": {
                    "type": "string",
                    "minLength": 1
                },
                "thumbNail": {
                    "type": "string",
                    "maxLength": 300,
                    "minLength": 1
                },
                "title": {
                    "type": "string",
                    "maxLength": 300,
                    "minLength": 1
                }
            }
        },
        "modules.YouTubeDenySettingsSongs": {
            "type": "object",
            "required": [
                "id",
                "thumbNail",
                "title"
            ],
            "properties": {
                "id": {
                    "type": "string",
                    "maxLength": 300,
                    "minLength": 1
                },
                "thumbNail": {
                    "type": "string",
                    "maxLength": 300,
                    "minLength": 1
                },
                "title": {
                    "type": "string",
                    "maxLength": 300,
                    "minLength": 1
                }
            }
        },
        "modules.YouTubeDenySettingsUsers": {
            "type": "object",
            "required": [
                "userName"
            ],
            "properties": {
                "userId": {
                    "type": "string",
                    "maxLength": 50
                },
                "userName": {
                    "type": "string"
                }
            }
        },
        "modules.YouTubeSettings": {
            "type": "object",
            "required": [
                "acceptOnlyWhenOnline",
                "announcePlay",
                "denyList",
                "enabled",
                "song",
                "translations",
                "user"
            ],
            "properties": {
                "acceptOnlyWhenOnline": {
                    "type": "boolean"
                },
                "announcePlay": {
                    "type": "boolean"
                },
                "channelPointsRewardId": {
                    "type": "string",
                    "maxLength": 100
                },
                "denyList": {
                    "$ref": "#/definitions/modules.YouTubeDenyList"
                },
                "enabled": {
                    "type": "boolean"
                },
                "maxRequests": {
                    "type": "integer",
                    "maximum": 500
                },
                "neededVotesVorSkip": {
                    "type": "number",
                    "maximum": 100,
                    "minimum": 1
                },
                "song": {
                    "$ref": "#/definitions/modules.YouTubeSongSettings"
                },
                "translations": {
                    "$ref": "#/definitions/modules.YouTubeTranslations"
                },
                "user": {
                    "$ref": "#/definitions/modules.YouTubeUserSettings"
                }
            }
        },
        "modules.YouTubeSongSettings": {
            "type": "object",
            "properties": {
                "acceptedCategories": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "maxLength": {
                    "type": "integer",
                    "maximum": 86400
                },
                "minLength": {
                    "type": "integer",
                    "maximum": 86399,
                    "minimum": 0
                },
                "minViews": {
                    "type": "integer",
                    "maximum": 10000000000000
                }
            }
        },
        "modules.YouTubeSongTranslations": {
            "type": "object",
            "properties": {
                "ageRestrictions": {
                    "type": "string"
                },
                "alreadyInQueue": {
                    "type": "string"
                },
                "cannotGetInformation": {
                    "type": "string"
                },
                "denied": {
                    "type": "string"
                },
                "live": {
                    "type": "string"
                },
                "maxLength": {
                    "type": "string"
                },
                "maximumOrdered": {
                    "type": "string"
                },
                "minLength": {
                    "type": "string"
                },
                "minViews": {
                    "type": "string"
                },
                "notFound": {
                    "type": "string"
                },
                "requestedMessage": {
                    "type": "string"
                }
            }
        },
        "modules.YouTubeTranslations": {
            "type": "object",
            "properties": {
                "acceptOnlyWhenOnline": {
                    "type": "string"
                },
                "channel": {
                    "$ref": "#/definitions/modules.YouTubeChannelTranslations"
                },
                "noText": {
                    "type": "string"
                },
                "notEnabled": {
                    "type": "string"
                },
                "nowPlaying": {
                    "type": "string"
                },
                "song": {
                    "$ref": "#/definitions/modules.YouTubeSongTranslations"
                },
                "user": {
                    "$ref": "#/definitions/modules.YouTubeUserTranslations"
                }
            }
        },
        "modules.YouTubeUserSettings": {
            "type": "object",
            "properties": {
                "maxRequests": {
                    "type": "integer"
                },
                "minFollowTime": {
                    "type": "integer"
                },
                "minMessages": {
                    "type": "integer"
                },
                "minWatchTime": {
                    "type": "integer"
                }
            }
        },
        "modules.YouTubeUserTranslations": {
            "type": "object",
            "properties": {
                "denied": {
                    "type": "string"
                },
                "maxRequests": {
                    "type": "string"
                },
                "minFollow": {
                    "type": "string"
                },
                "minMessages": {
                    "type": "string"
                },
                "minWatched": {
                    "type": "string"
                }
            }
        },
        "null.String": {
            "type": "object",
            "properties": {
                "string": {
                    "type": "string"
                },
                "valid": {
                    "description": "Valid is true if String is not NULL",
                    "type": "boolean"
                }
            }
        },
        "parser.GetVariablesResponse_Variable": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "example": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "visible": {
                    "type": "boolean"
                }
            }
        },
        "spotify.SpotifyProfile": {
            "type": "object",
            "properties": {
                "country": {
                    "type": "string"
                },
                "display_name": {
                    "type": "string"
                },
                "email": {
                    "type": "string"
                },
                "explicit_content": {
                    "type": "object",
                    "properties": {
                        "filter_enabled": {
                            "type": "boolean"
                        },
                        "filter_locked": {
                            "type": "boolean"
                        }
                    }
                },
                "external_urls": {
                    "type": "object",
                    "properties": {
                        "spotify": {
                            "type": "string"
                        }
                    }
                },
                "followers": {
                    "type": "object",
                    "properties": {
                        "href": {
                            "type": "string"
                        },
                        "total": {
                            "type": "integer"
                        }
                    }
                },
                "href": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "images": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "properties": {
                            "height": {
                                "type": "integer"
                            },
                            "url": {
                                "type": "string"
                            },
                            "width": {
                                "type": "integer"
                            }
                        }
                    }
                },
                "product": {
                    "type": "string"
                },
                "type": {
                    "type": "string"
                },
                "uri": {
                    "type": "string"
                }
            }
        }
    },
    "securityDefinitions": {
        "ApiKeyAuth": {
            "type": "apiKey",
            "name": "api-key",
            "in": "header"
        }
    }
}`

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo = &swag.Spec{
	Version:          "1.0",
	Host:             "localhost:3002",
	BasePath:         "/",
	Schemes:          []string{},
	Title:            "Tsuwari api",
	Description:      "Non-public api for tsuwari",
	InfoInstanceName: "swagger",
	SwaggerTemplate:  docTemplate,
}

func init() {
	swag.Register(SwaggerInfo.InstanceName(), SwaggerInfo)
}
