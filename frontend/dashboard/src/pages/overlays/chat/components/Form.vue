		...
		<div class="card-body">
			<div class="card-body-column">
				...
				<div v-if="formValue.preset === 'boxed'" class="switch">
					...
					<NSwitch v-model:value="formValue.showAnnounceBadge" :disabled="!formValue.showBadges" />
				</div>

				<div class="slider">
					<span
						>{{ t('overlays.chat.paddingContainer') }} ({{ formValue.paddingContainer }}px)</span
					>
					<NSlider
						v-model:value="formValue.paddingContainer"
						:min="0"
						:max="256"
						:marks="{ 0: '0', 256: '256' }"
					/>
				...
				<div>
					...
					<NSelect v-model:value="formValue.fontWeight" :options="fontWeightOptions" />
				</div>

				<div>
					<span>{{ t('overlays.chat.fontStyle') }}</span>
					<NSelect v-model:value="formValue.fontStyle" :options="fontStyleOptions" />
				</div>
				...
				<div class="slider">
					...
					<NColorPicker v-model:value="formValue.textShadowColor" default-value="rgba(0,0,0,1)" />
				...
				<div class="slider">
					...
					<NSlider v-model:value="formValue.messageHideTimeout" :max="60" :marks="sliderMarks" />
				</div>

				<div class="slider">
					<span>{{ t('overlays.chat.showDelay') }}({{ formValue.messageShowDelay }}s)</span>
					<NSlider v-model:value="formValue.messageShowDelay" :max="60" :marks="sliderMarks" />
				</div>
			</div>
		</div>
	</div>
