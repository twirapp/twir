...
interface Crud<T> {
	...
	useCreateOrUpdate: () => UseMutationResult<
		any,
		unknown,
		{ id?: string | undefined; data: T },
		unknown
	>;
}

const createCrudManager = <T extends { id: string }>(
	system: string,
	opts?: {
		...
		useDelete: () =>
			useMutation({
				mutationFn: (id: string) => {
					return authFetcher(`${getUrl(system)}/${id}`, {
						method: 'DELETE',
						headers: {
...
export const categoriesAliasesManager = () =>
	createCrudManager<ChannelCategoryAlias>('categories-aliases');
...
export const commandsGroupManager = () =>
	createCrudManager<ChannelCommandGroup>('commands/groups', {
