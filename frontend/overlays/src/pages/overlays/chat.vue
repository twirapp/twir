...
const { overlaySettings: settings, neededData, chatLibSettings, chatEvents } = useChatOverlaySocket()
...
watch(chatEvents, (v) => {
	const event = v?.overlaysChatEvents
	if (!event) return

	if (event.__typename === 'ChatOverlayMessage') {
		...
		pushMessage({
			...
			senderColor: event.senderColor ?? undefined,
	...
	if (event.__typename === 'ChatOverlayModerationEvent') {
		switch (event.type) {
