...
import { useVueTable, FlexRender, createColumnHelper, getCoreRowModel, SortingState } from '@tanstack/vue-table';
import { type GetUsersResponse_User } from '@twir/grpc/generated/api/api/community';
import { computed, h, ref } from 'vue';

import { useCommunityUsers, useTwitchGetUsers, type SortKey } from '@/api/index.js';
...
<template>
	<div style="display: flex; justify-content: flex-end; gap: 8px">
		<div class="text-slate-200">
			Total	{{ data?.totalUsers }} users | Page {{ page }} of {{ pagesCount }}
		</div>
		<button
			:disabled="page === 1"
			class="border rounded-md"
			:class="[page === 1 ? 'border-slate-700' : 'border-slate-200']"
			@click="page--"
		>
		...
		<button
			class="border rounded-md"
			:class="[page === pagesCount ? 'border-slate-700' : 'border-slate-200']"
