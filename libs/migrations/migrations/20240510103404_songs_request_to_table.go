...
func upSongsRequestToTable(ctx context.Context, tx *sql.Tx) error {
	...
	_, err = tx.ExecContext(
		...
		`CREATE TABLE IF NOT EXISTS channels_song_requests_settings (
			...
			accept_only_when_online BOOLEAN NOT NULL,
			player_no_cookie_mode BOOLEAN NOT NULL,
			...
			announce_play BOOLEAN NOT NULL,
			needed_votes_for_skip FLOAT CHECK (needed_votes_for_skip <= 100 AND needed_votes_for_skip >= 1),
			user_max_requests INT,
			user_min_watch_time BIGINT,
			user_min_messages INT,
			user_min_follow_time INT,
			song_min_length INT CHECK (song_min_length >= 0 AND song_min_length <= 86399),
			song_max_length INT CHECK (song_max_length <= 86400),
			song_min_views INT CHECK (song_min_views <= 10000000000000),
			song_accepted_categories VARCHAR[] CHECK (array_length(song_accepted_categories, 1) <= 300),
			song_words_deny_list VARCHAR[] CHECK (array_length(song_words_deny_list, 1) <= 300),
			deny_list_users VARCHAR[] CHECK (array_length(deny_list_users, 1) >= 1),
			deny_list_songs VARCHAR[] CHECK (array_length(deny_list_songs, 1) >= 1),
			deny_list_channels VARCHAR[] CHECK (array_length(deny_list_channels, 1) >= 1),
			deny_list_artists_names VARCHAR[] CHECK (array_length(deny_list_artists_names, 1) <= 300),
			deny_list_words VARCHAR[] CHECK (array_length(deny_list_words, 1) <= 300),
